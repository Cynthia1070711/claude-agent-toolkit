...
> ★ create（規格產出）→ dev（規格消費）→ review（規格驗證）· 38 步逐項 + 測試職責分工
...
> **部署注意**：BMAD 本體需使用者**自行安裝新版**，本包只提供客製化覆蓋層（見 §6.2）。

### 6.2 ★ 三大開發流程 —— 已非官方原版

`_bmad/bmm/workflows/4-implementation/` 下的 create-story / dev-story / code-review
**已被本專案深度改寫**，範本 `bmad-overlay/4-implementation/`（73 檔）即為此覆蓋層。
最近一輪重大升級為 **epic-bwu「BMAD 規格開發閉環升級」**，16 張 Story 全數完成
（`bwu-1` ~ `bwu-15` + 母卡，最後更新 2026-08-04）。

#### 6.2.0 共同架構：step-file

三個 workflow 皆採 **step-file 架構**，而非單一長檔：

| 特性 | 說明 |
|------|------|
| **每步驟獨立載入** | 對抗 LLM "lost in the middle" —— 執行到哪就只載哪一步 |
| **STATE VARIABLES 跨步驟持久化** | 如 `{story_key}` `{diff_output}` `{review_mode}` `{unified_findings}` |
| **步驟鏈明示** | 每個 step 檔 frontmatter 標 `nextStepFile`，不靠模型自由發揮 |

步驟數：**create-story 9 步 · dev-story 14 步 · code-review 15 步**。

#### 6.2.1 ★ 測試職責三層分工（由模型分層驅動）

**這是本環境最重要、也最容易被誤解的設計決策：測試「設計」與測試「實作」是分開的，
由不同階段、不同能力的模型分別負責。**
...
| 階段 | 模型 | effort | 裁定 |
|------|------|:------:|------|
| **create-story** | `claude-opus-5[1m]` | max | 2026-06-08 使用者裁定 |
| **dev-story**（含 complex / fix-Rn） | `claude-sonnet-5[1m]` | max | 2026-07-19 使用者裁定（supersede 2026-06-11 opus 裁定） |
| **code-review** | `claude-opus-5[1m]` | max | 2026-06-08 使用者裁定 |

##### 為什麼測試設計必須在 create 階段

2026-03-08 的原始決策是「Test Skeleton 省略獨立產出，dev 直接從 BR 生成測試」，
理由是「減少一層文件轉換」。**該決策在當時完全成立 —— 因為三階段是同一個模型，誰寫都一樣。**

2026-06-08 模型分層後，前提失效：
...
直接把測試檔搬到 create 有三項真實代價：破壞 create 的 DB-first 零產檔性質、
簽章未定必然導致猜測、與 dev §0.4 職責重疊。**故裁定為分離「設計」與「實作」**：
...
##### create 端：`step-06-create-story-file.md` §7.5 測試規格產出

**分級 Gate**（全自動依 DB `complexity` 判定，禁引入任何 `<ask>`）：
`S` / `XS` → 完全跳過，維持自由格式；**`M` / `L` / `XL` → 強制產表**。

**七欄必填 Schema**（+ 選填第八欄 `Pattern`）：

| 欄位 | 內容規範 |
|------|---------|
| `Case` | `{BR_ID}_{Scenario}_{ExpectedResult}` —— 引用 dev step-06:30 既有命名法，不另立。BR id 去分隔符後需 ≥2 段底線（`BR001_Works` 因段數不足視為不合法） |
| `BR` | 本案例驗證的 BR id |
| `Level` | 僅能是 `unit` \| `integration` \| `E2E` |
| `Fixture` | **具名既有資產**（`CustomWebApplicationFactory` / `TestcontainersWebApplicationFactory` / `TestAccountSeeder A1-A5` …）或 `N/A — pure function`；禁空白或非具名描述 |
| `Input` | 具體輸入值，非描述性文字 |
| `Expected` | 具體預期輸出 / HTTP 狀態碼 / 錯誤碼 |
| `RED→GREEN` | 雙向皆具體（如 `RED: 回 200（無版本檢查）/ GREEN: 回 409`），**禁**「會通過」「應該正常」類模糊敘述 |

**BR 覆蓋要求**：`acceptance_criteria` 中每個 `[Verifies: BR-XXX]` 的 BR id，
表格必含 ≥1 列對映（比對前正規化：轉大寫 + 去非英數字元）。
...
**Pattern-first**：產表前先比對 `test-spec-pattern-catalog.md`，命中則只寫參數差異並標
`[Pattern: {ID}]`，不適用時須附**具名豁免行**禁靜默省略；目錄不可讀時降級為「未命中」續行，不中斷 workflow。

##### dev 端：`step-05-implement-task.md` §0.4 —— 從「自行推導」改為「消費」
...
加上 **`Case` 名稱必須 byte-identical**（不得重新命名、不得意譯、不得為了「更好懂」而調整），
dev 的職責被收斂為**純翻譯與實作**，四項設計決策全由 create 階段的表提供。

**RED 判定**：依該列 `RED→GREEN` 的 RED 側描述；若觀察到的是編譯錯誤或 `NullReferenceException`，
判定為**測試寫錯而非實作缺失**，須先修正測試再重跑。

**重入（`dev-story-fix-Rn`）只補漏不重寫**：CLI 回傳每列 `testHit`（`file:line` 或 `null`），
非 null 的列跳過，只為 `null` 的列補寫。輸出 `consume: {N} new, {M} existing`。
...
#### 6.2.2 create-story（9 步 · opus-5 max）

| # | 步驟 | 職責（原文 Goal） | 調用 Skill / 腳本 |
|:-:|------|------------------|------------------|
| 1 | `step-00-db-first-query` | 讀任何 `.md` **之前**先查 Context Memory DB 既有 Story 資料 | `record-phase-timestamp.js` |
| 2 | `step-01-target-story` | 決定要建哪張卡（使用者指定或 DB 自動探索） | `record-phase-timestamp.js` |
| 3 | `step-02-artifact-analysis` | 窮舉分析所有 artifact，抽取關鍵脈絡 | — |
| 4 | `step-03-codebase-analysis` | ★ **對照實際實作驗證 tasks，防重複開發 / 漏掉既有抽象 / 檔案位置錯誤 / regression** | `search_god_nodes` MCP |
| 5 | `step-04-architecture-analysis` | 抽出架構文件中開發者 MUST 遵守的約束 | — |
| 6 | `step-05-web-research` | 確保技術知識為最新，防過時實作 | WebFetch / WebSearch |
| 7 | `step-06-create-story-file` | 寫出「終極開發者指南」進 DB，含 **§7.5 測試規格產出**（bwu-2） | `sdd-spec-generator` · `phycool-sqlserver` · `phycool-testing-patterns` · `verification-before-completion` · `story-status-emoji` |
| 8 | `step-06.5-depth-gate` | ★ 5 維形式閘門後再跑 **7 項深度實質驗證** | `phycool-create-story-depth-gate` + `run-depth-gate.js` · `upsert-story.js` |
| 9 | `step-07-finalize` | 對照 checklist、同步 DB（含 `track_plan`）、建 tracking 檔 | `upsert-story.js` · `upsert-track-plan.js` · `record-phase-timestamp.js` |
...
**★ step-03 三層防重複機制**（使用者關切的「建構前先檢查是否已有重複功能」）：

1. **Stale Data Detection** —— 信任任何資料檔（error log / tsc-errors.txt / lint 報告 / 測試結果）前先比對修改時間，比最新 commit 舊即為 STALE，須逐條對照實際原始碼；**絕不從報告複製行號**而不開檔確認
2. **God Node Priority Scan**（2026-05-02 加入）—— 在 Glob/Grep 多目錄掃描**之前**，先以 domain keyword 呼叫 `mcp__phycool-context__search_god_nodes`，取 top-N 高 centrality 核心檔優先 Read，並把 BlastRadius 大的 symbol 標進 dev_notes；0 hit 時 fallback 回既有 Glob/Grep
3. **Mandatory Source File Verification** —— **GATE：Story 提到的每一個原始檔都被 READ 並驗證過之前，不得進入 Step 4**

**Depth Gate 7 項深度驗證**（`step-06.5`）：Skill Read / ADR+IDD Cross-Ref / PRD Source /
Chrome MCP Live / Iteration Pollution / Cross-Story / **Self-Write Verification**（驗 DB 寫入無
bash heredoc 或 template 損壞）。WARN 等同 BLOCK，除非 `--accept-warn "具體理由"`。

**DB-first**：Story 全部存 DB，**禁止**產生 `docs/.../{story_id}.md` 鏡像
（`db-first-write-guard.js` PostToolUse 攔截，`source_file` 填 `context-db://stories/{id}`）。

**相關 Rules**：`create-story-enrichment` · `db-first-no-md-mirror` ·
`depth-gate-warn-mandatory-resolution` · `parallel-batch-conflict-isolation`

#### 6.2.3 dev-story（14 步 · sonnet-5 max）

| # | 步驟 | 職責（原文 Goal） | 調用 Skill / 腳本 |
|:-:|------|------------------|------------------|
| 1 | `step-00-violation-precheck` | ★ 開工**前**強制 `Read` 最近 30 天 top-3 熱點違規規則全文 | `query-violations.js` |
| 2 | `step-00-db-first-query` | 讀 `.md` 前先查 DB 取 Story 脈絡 | — |
| 3 | `step-01-load-story` | 定位並完整載入 Story，建立 state variables | — |
| 4 | `step-02b-kb-precheck` | 依本卡 domain/skills 掃知識庫既有已知問題 | 12 個 `phycool-*` 模組 Skill（依 domain 命中） |
| 5 | `step-02-load-context` | 載入專案模式、required skills（含**過時偵測**）、前置技術債 | `{staleness_hits}` / `{incoming_tech_debt}` |
| 6 | `step-03-review-continuation` | 判定全新開始 or CR 後續修復（fix-Rn） | — |
| 7 | `step-04-mark-in-progress` | 確保 tracking 檔存在 + 記 dev 起始時間戳（**status 由 pipeline dispatcher 擁有，本步不寫**） | `record-phase-timestamp.js` |
| 8 | `step-05-implement-task` | ★ 依 red-green-refactor 實作；**§0.4 消費 create 端測試案例表** | `test-spec-audit.js` |
| 9 | `step-05b-apply-migrations` | 套 EF Core migration + 跑 Migration Cascade Checklist（ADR-DB-001） | — |
| 10 | `step-06-author-tests` | **覆蓋率 Gate + cross-task 補漏**（非重寫，見 §6.2.1） | `phycool-e2e-playwright` |
| 11 | `step-07-run-validations` | 跑全部測試與 lint，確保無 regression、AC 全滿足 | — |
| 12 | `step-08-validate-complete` | 任務完成閘門 —— 所有條件齊備才可標完成 | — |
| 13 | `step-09-completion` | 驗 DoD、標 review、跑 bug 驗證、tasks 回填、文檔漂移偵測 | `bug-fix-verification` · `tasks-backfill-verify` · `story-status-emoji` · `saas-to-skill` + `test-spec-audit.js` · `upsert-story.js` · `boy-scout-sweep.js` · `check-violation-repeat.js` · `log-rule-violation.js` |
| 14 | `step-10-communication` | 記錄 workflow 完成、彙總成果、建議下一步 | `log_workflow` MCP |

**相關 Rules**：`capability-integration-mandate` · `context-memory` · `db-first-no-md-mirror` ·
`skill-sync-gate` · `testing`

**★ Workflow Entry Gate（`step-00-violation-precheck`）** —— 這是最容易被忽略的機制：

開工前先跑 `query-violations.js --phase dev-story --since-days 30`，
取最近 30 天該階段的 **top-3 熱點違規規則**，**強制 Read 其全文**後才准進入 step-01。
`{precheck_violation_count}` 為 0 時短路跳過。code-review 有同名同機制的入口閘門。

> 這條讓「違規學習閉環」（§4.3）從被動注入升級為**主動閘門**：
> 不只是在 prompt 裡看到違規熱區，而是開工前被強制讀完那幾條規則。
...
#### 6.2.4 code-review（15 步 · opus-5 max）—— 六層平行 + Review Trail
...
**相關 Rules**：`cr-debt-doc-audit` · `db-first-no-md-mirror` · `skill-idd-sync-gate` ·
`skill-sync-gate` · `tasks-backfill`
...
**設計精神**：A 層刻意「盲」—— 不給 spec，才能發現「spec 沒說但明顯錯」的問題；
C 層才對照驗收條件。兩者互補，避免單一視角的確認偏誤。

**Step 3d Triage Merge**（bwu-7 升級）：消費**全部 7 個 finding 來源**（A/B/C/D/E/F + SaaS），
統一去重計分，產出 `{unified_findings}` + `{dismissed_count}` + `{saas_readiness_score}`。

**Step 4 Auto-fix 強制**：`template: false`（action-workflow），Steps 1→5 **連續執行不詢問**，
不得在步驟間插入 `[c]/[y]/[a]` 確認 —— auto-fix 是 MANDATORY，永不請求修復許可。

**Guard 條件**：`{diff_output}` 為空 → 「Nothing to review」HALT；diff > 3000 行 → 警告但續跑。

#### 6.2.5 規格驅動開發的文檔規範與防重複機制
...
- ❌ **測試規格表** —— 由 create-story `step-06` §7.5 產出並寫入 `stories.testing_strategy`
  （SKILL.md 明文標註此為 2026-06-08 模型分層裁定的結果，見 §6.2.1）
- ❌ **Test Skeleton** —— 原「dev 直接從 BR 生成」的決策已被前述裁定 supersede

實測 `docs/implementation-artifacts/specs/` 下有 **117 個 spec 檔** + epic 子目錄分類
（`backend-vision/` · `content-editing/` 等）。

##### 規格層的 signature-free 原則

`sdd-spec-generator` 品質門檻 #7 明文：

> **不含實作細節 —— Spec 說 WHAT，不說 HOW（無 C# class name、無 method signature）**

這條原則向下傳遞：因為 Spec 層刻意 signature-free，在其上長出的測試案例表也維持此性質
（§7.5 產「表」不產「檔」的理由之一 —— create 階段介面與 DI 尚未存在，任何簽章都是猜測）。

##### 建構前防重複：四道關卡

使用者關切的「建構前先檢查是否已經有重複功能規範」，在本環境有四個層次的機制：
...
| **3** | create-story `step-03-codebase-analysis` | 建卡分析階段 | God Node Priority Scan + Stale Data Detection + **Mandatory Source File Verification GATE**（見 §6.2.2） |
| **4** | `scripts/audit-skill-overlap.cjs` | Skill 新建 / 定期稽核 | 偵測 Skill 之間的職責重疊 |

**`pre-audit-mandate` 的 6 條 FORBIDDEN**（防合理化繞過）：
...
同一 rule 的 **§3.5 Commit Pre-Check** 另有 5 題自答（`git status` / `git diff --staged` /
並行 agent 檢查 / scope verify / message verify），由 `pre-commit-quality.js` hook 的
BR-004/005/006 機械守護。

##### 測試規格的重複防護

create `step-06` §7.5 的 **Pattern-first 子步**是同一防重複精神在測試層的展現：

產表前先比對 `.claude/skills/phycool-testing-patterns/references/test-spec-pattern-catalog.md`，
**命中** → 該列標 `[Pattern: {ID}]`，只寫參數差異（endpoint / auth scheme / fixture 帳號），
不重複描述整套標準案例；某標準案例本卡不適用時**必附具名豁免行**（`豁免: {案例名} — {理由}`），
禁靜默省略。**未命中** → 全套寫出並記為晉升候選，供 code-review 階段第二次同型場景出現時晉升入目錄。

#### 6.2.6 epic-bwu 帶來的 16 項升級（實測全 done）
...
workflow 內容不是「文件寫了就算」—— 有測試檔約束其結構：
...
> **重要**：`workflow-invoked-detect.js` 用 `BMAD_PATH_RE = /_bmad[\\/].*workflows[\\/]/i`
> 判定「本次是否真的走了 BMAD workflow」，這是 Pipeline 六證據鏈中 `workflow_invoked=true` 的來源。
> 手動宣稱走了流程但沒實際載入 step 檔，這裡會抓到。
...
← [能力封裝（Skills + Commands + Agents）](04-能力封裝.md) · [總表](../00-開發環境架構清單.md) · [工作流 · Pipeline 與跨軌通訊](06-工作流-Pipeline與跨軌.md) →
