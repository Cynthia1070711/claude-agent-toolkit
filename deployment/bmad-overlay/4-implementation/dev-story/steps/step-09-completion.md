...
4. Execute enhanced definition-of-done validation:
   ...
   - Story 資訊 table `**狀態**` row is set to "review"(DB-first Story:`stories.status` 為唯一 SSoT)

### 1.5 測試案例名漂移 Advisory(M/L/XL · 不 BLOCK)

> **CRITICAL:** 🔍 dev-story 標 review **之前**的最後一道機械回饋 —— 對照 code-review `step-03c-acceptance-auditor.md` §6.2 軸 (a)(byte-identical)**同一判準**(whole-word),補上 dev 端當場可見的 stderr advisory,取代原本只在 CR 才被發現的落差(`bwu-13-bmad-mechanism-gap-closure` BR-001~BR-008)。

**觸發**: M/L/XL complexity Story(S/XS 跳過 — 本節同 §0.4 分級 gate)。

**執行指令**:

```bash
node .context-db/scripts/test-spec-audit.js {story_key} --dev-advisory
```

- stderr 有輸出 → 逐列顯示 `⚠ 測試案例名漂移 advisory` 摘要行 + 每列 `  [{rowIndex}] {BR}  {Case}`
- stderr 為空 → 無漂移(或 verdict 非 `consume`),直接繼續

> **執行成本**:本指令對每個案例列各跑一次 `git grep --untracked`(全 repo 掃描),41 列的卡實測約 40 秒;這是**本階段第二次**全 repo 掃描(§0.4 的 `--json` 為第一次)。屬預期成本,勿因耗時而跳過。

**對每個列出的未命中列,二擇一**(**禁靜默忽略**):

1. **改測試方法名** 對齊 `testing_strategy` 表的 `Case` 值(byte-identical),或
2. **於 `dev_notes` 具名記錄刻意分歧的理由**(例如:表該補一列而非改名,對齊 §0.4.2 案例名契約不得單方面繞過)

**本 advisory 不阻斷流程**:exit code 恆為 `0`,`--dev-advisory` 不改變 stdout / exit code(僅 stderr 額外輸出),Story 仍可正常進入 Step 2 標記 review。
...
> **DB-first (bwu-1):** 僅當本 Story 確有 legacy `.md` 鏡像時適用 —— 此時調用 `Skill(skill="story-status-emoji")` Mode A（status: review）。
> `{db_context_available}` == true 的 DB-first Story **無 `.md` 無 H1 標題**，本步驟屬 **N/A**，狀態以 DB `stories.status` 為 SSoT。
> **不得**為了套用 emoji 而產出 `.md`（違反 SUPREME `.claude/rules/db-first-no-md-mirror.md`）。
...
3. **Produce Skill Impact Report**:
   ...
   |---------------|-----------------|-------------|--------|
   | phycool-xxx | L42: section name | Business rule | ✅/⬜ |
   ```
...
**Quick Decision:**
- `{file_list}` contains Migration/Model/Service/Controller/Route/Component changes? → Grep search phycool-* Skills
...
1. **Assemble session-actions JSON** (Agent self-constructed, no infra dependency):
   ...
   - **`tool_calls` scan surface (narrowed 2026-07-27 by `td-devenv-guard-gate-false-signal-repair`):** only these string-valued `input` fields are scanned as prose — `command` / `old_string` / `new_string` / `content` / `prompt` / `description` / `summary` / `message`. Structural identifiers (`name`, `skill`, `file_path`, …) are deliberately excluded, which is what stopped a compliant `Skill(skill="tasks-backfill-verify")` call from reading as a tasks-backfill mention. **Consequence:** prose placed under any other field name is silently unscanned — put decision language in one of the eight fields above.
...
> **No output-file flag, by design**: `check-doc-drift.ps1`'s output-file parameter defaults to an empty string, and both of its output sites are `if (<param set>) { Out-File } else { Write-Host $Json }` — omitting the flag makes the script emit its JSON to stdout instead of writing a file. `-Quiet` only suppresses the script's own `Write-Drift` progress lines; it does not suppress the direct `Write-Host $Json` call. This mirrors the equivalent invocation already in `code-review/steps/step-06-report-archive.md` §3, which never wrote a file and parses stdout directly.

Parse drift results directly from the command's stdout JSON:
...
**If `{drift_affected_docs}` is empty or the script itself is not found (command fails to run):**
...
- All tasks marked `[x]`
- Story status set to "review" in `stories` table(DB 單一 SSoT;sprint-status.yaml 已凍結 2026-07-28,不再同步)
...
- **Not updating stories table via upsert-story.js** (DB-first 架構違反,唯一 SSoT)
