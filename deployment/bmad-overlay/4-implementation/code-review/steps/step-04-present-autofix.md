...
- `{story_key}`, `{tech_debt_count}`, `{saas_readiness_score}` — from Steps 1/3d
- `{unified_findings}` — deduped unified Finding[] from Step 3d (source: blind/edge/auditor/security-expert/perf-expert/db-reviewer/saas/merged)
...
| Security Expert (3-D) | N | OWASP 12 維 + 5 PhyCool 專屬 |
| Perf Expert (3-E) | N | .NET/EF Core/SQL Server/React 效能 10 維 |
| DB Reviewer (3-F) | N | Migration/PK/Index/Null/Spec/Retention 6 項(僅 `{has_db_changes}` = true) |
| SaaS Audit (main) | N | 10-dimension safety net |
...
> **REF:** `phycool-debt-registry` §2.3, §5, §6, §13 — ACCEPTED, Priority Score, 5-Min Rule, Production Gate.
> **REF:** `phycool-intentional-decisions` §1.4, §9.4 — IDD Sub-Types, IDD Detection Gate.
...
> **REF:** `phycool-debt-registry` §5

#### Phase 0.5 (NEW 2026-05-02 ADR-GOVERNANCE-001) — God Node BlastRadius Auto-Lookup

> **Added 2026-05-02 (ADR-GOVERNANCE-001 + Tianji v1.1.0)**: BlastRadius **不可主觀估算**(對齊 `cr-debt-doc-audit.md` Phase A2「禁止以估算成本替代實際試修」精神)。對每個 non-FIXED finding,先用 god node MCP 自動補 BlastRadius。
...
**使用**: `Priority Score = Severity × BlastRadius_auto_lookup × BusinessImpact ÷ FixCost`,**不可繞過**(對齊 F-S9 投機紅線)。
...
> **REF:** `phycool-intentional-decisions` §9.4 — code-review Step 3.5 IDD Detection Gate
...
- **執行完整 8 步 IDD 建立流程:**
  ...
  5. 更新 `phycool-system-platform` 對應 module 文件
...
- **B. 路由決策** (per MEMORY.md: CR deferred items must stay in original Epic):
  1. Query `search_stories({epic_id: "{epic}", status: "backlog", fields: "story_id,title"})` for same-Epic Story;若空再以 `status: "ready-for-dev"` 查一次(do NOT read Story full text)
...
- ☐ Source Story Tech Debt Reference section updated(`tech_debt_items` 表為 SSoT,sprint-status.yaml 已凍結 2026-07-28 不再同步)
...
- upsert-debt.js called for ALL DEFERRED/WON'T FIX/ACCEPTED items
- Push Checklist all passed (11 items)
