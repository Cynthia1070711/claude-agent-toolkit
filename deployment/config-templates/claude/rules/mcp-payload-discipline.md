...
| 記錄 rule_violation(任何情境,含非 CR) | 必走 `node .context-db/scripts/log-rule-violation.js`(canonical JSON content),**禁**手動 `add_context` 寫 markdown 進 `category='rule_violation'` |
...
- ❌ **`add_context` 寫 `category='rule_violation'` 但 content 為 markdown(非 JSON)** — 必走 `node .context-db/scripts/log-rule-violation.js`(canonical schema:`violated_rule_path`/`workflow_phase`/`severity`/`incident_summary`/`rule_loaded_at_time`/`cli_enforcement`)。手動 markdown 會讓 `pre-prompt-rag.js` Layer 11 `getRecentHotViolations()` 的 `json_extract(content,'$.violated_rule_path')` throw → 整查 catch return `[]` → 違規熱區靜默斷鏈(**G19 重複違規根因**,2026-05-25)。機械守護見 `.claude/hooks/rule-violation-write-guard.js`(Part 2b)
...
3. **timestamp 是 offset-aware 嗎?** (`Get-TaiwanTimestamp` (PS) / `getTaiwanTimestamp()` (`.context-db/scripts/timezone.js`) — 皆產 `+08:00`;**禁 naive** `toLocaleString('sv',{timeZone:'Asia/Taipei'})`。對齊 constitutional §Timestamp Mandate offset-aware 原則)
...
- **2026-05-25 G19 rule_violation 斷鏈根治 (v1.1.0)**: 4 筆手動 `add_context` markdown(`json_valid=0`)→ Layer 11 `json_extract` throw → catch return `[]` → 違規熱區靜默斷鏈(重複違規無警示)。修復:核心 `json_valid` guard(commit `70bdc0ca`)+ 回填 3 筆/正規化 2 筆 + 本 §4 FORBIDDEN #9(填補 `cr-debt-doc-audit.md` Phase D 僅 CR 情境 gap)+ `rule-violation-write-guard.js` 機械守護。
- **歷史式錯實例** (本 rule 防範對象):
...
| 版本 | 日期 | 變更 |
|:---:|:---:|------|
| **1.1.0** | **2026-05-26** | G19 斷鏈根治 Part 2a。§3 矩陣加「rule_violation 必走 log-rule-violation.js」+ §4 FORBIDDEN #9(禁手動 add_context markdown 進 rule_violation)+ §8 Incident G19。填補 `cr-debt-doc-audit.md` Phase D 僅 CR 情境 gap。配套 Part 2b `rule-violation-write-guard.js` PreToolUse 機械守護。 |
