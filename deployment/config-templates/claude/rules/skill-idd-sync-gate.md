...
| 4 | phycool-system-platform module 文件更新 | references/**.md 修改 |
...
2. **若任一為 Yes**:
   ...
   - **必須**執行:
     ...
     6. 更新 phycool-system-platform 對應 module 文件
3. **若 Q1-Q4 全為 No**:
   - 繼續 phycool-debt-registry 既有流程 (5-Min Rule → 分類)
...
## phycool-system-platform 同步檢查

Skill-IDD Sync Gate 的 **延伸觸發**: 若 IDD 的 `platform_modules` 非空,必須額外檢查:

| IDD 變更 | 必須同步的 phycool-system-platform 檔案 |
|---------|------------------------------------------|
...
**風險**: 新對話載入 stale phycool-system-platform → 以為某功能是「可修」→ 實際是 IDD → 誤修 → 重複白工
