...
async function main() {
  ...
  console.log(`\n=== PhyCool Context Memory DB Validation ===`);
  ...
  if (docTotal > 0 && parseFloat(docCov) < 95) {
    issues.push({ table: 'document_embeddings', issue: `doc embedding coverage ${docCov}% < 95% (${docTotal - docEmbed} chunks 缺向量,跑 backfill-doc-embeddings.js 補)` });
  }
