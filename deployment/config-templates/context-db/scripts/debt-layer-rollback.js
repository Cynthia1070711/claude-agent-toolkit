// ============================================================
// PhyCool Context Memory DB — Debt Layer Rollback
...
export function createBackup(label = 'dla08') {
  ...
  let filename = `phycool.db.backup-${today}-${label}`;
  let target = path.join(BACKUPS_DIR, filename);

  if (fs.existsSync(target)) {
    filename = `phycool.db.backup-${today}-${label}-${nowHHmm()}`;
...
export function listBackups() {
  ...
  const entries = fs.readdirSync(BACKUPS_DIR)
    .filter(f => f.startsWith('phycool.db.backup-'))
    .map(f => {
