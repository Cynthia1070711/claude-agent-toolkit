...
function estimateTokenUsage(db, sessionId) {
  ...
  try {
    // FIX (2026-05-29 · DB Schema-First Mandate): context_entries 真實 timestamp 欄 = `timestamp`(非 created_at · PRAGMA verified)
    //   原 created_at 不存在 → 整 try throw → 永遠走 fallback_zero。TZ 註:datetime(timestamp +08:00)→UTC vs datetime('now')→UTC
    //   兩側同 UTC,相對 2h 窗口本就正確(無需 +8h;+8h 反破壞為比未來時刻)。
