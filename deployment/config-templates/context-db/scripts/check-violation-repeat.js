...
export const DEFAULT_DB_PATH = path.resolve(__dirname, '..', 'phycool.db');
...
tool_calls scan surface (IMPORTANT — narrowed 2026-07-27):
  Only these string-valued input fields are scanned as prose:
    command, old_string, new_string, content, prompt, description, summary, message
  Structural identifiers (name, skill, file_path, path, ...) are deliberately NOT
  scanned — that is what made a compliant Skill(skill="tasks-backfill-verify") call
  look like it "mentioned" the tasks-backfill rule. Consequence: a free-text payload
  placed under any OTHER field name is silently unscanned. When emitting this file,
  put prose in one of the eight fields above.
...
export function buildFallbackPatterns(hotRulePaths, keywordMap) {
  ...
  for (const rulePath of hotRulePaths) {
    ...
    out.push({ rule: rulePath, kw: token, sev: 'low', kind: 'string', raw: token, origin: 'fallback' });
  }
  return out;
}
...
/**
 * Free-text fields within a tool_call's `input` where an agent could plausibly
 * write violation-describing prose (a Bash command, a code edit, a written
 * document). Deliberately excludes structural/identifier fields (`name` at the
 * tool_call level, or fields like `skill` / `file_path` / `path` within `input`)
 * — those name *what was invoked*, not *what was said*, and treating them as
 * scannable prose is what causes a compliant `Skill(skill="tasks-backfill-verify")`
 * call to look like it "mentions" tasks-backfill or a SKILL.md fallback token.
 */
const TOOL_CALL_FREE_TEXT_FIELDS = ['command', 'old_string', 'new_string', 'content', 'prompt', 'description', 'summary', 'message'];

/**
 * Build the scan text for a single tool_call: only its free-text input fields,
 * joined. Returns '' when the call has no free-text fields (e.g. Skill/Read-style
 * calls whose input is purely structural identifiers) — such calls are skipped
 * entirely by the scanner rather than matched against their serialized form.
 * @param {Object} tc - { name, input }
 * @returns {string}
 */
export function extractToolCallScanText(tc) {
  if (!tc || typeof tc !== 'object') return '';
  const input = tc.input;
  if (!input || typeof input !== 'object') return '';
  const parts = [];
  for (const field of TOOL_CALL_FREE_TEXT_FIELDS) {
    if (typeof input[field] === 'string' && input[field]) parts.push(input[field]);
  }
  return parts.join('\n');
}

/**
 * True for patterns produced by buildFallbackPatterns() (tagged `origin:'fallback'`
 * at creation time). NOT a shape guess (sev==='low' && kind==='string') — explicit
 * keyword-map entries can legitimately share that shape (e.g. "Boy Scout 跳過"),
 * and word-boundary-guarding those would break multi-char CJK phrases (a trailing
 * `\b` after a non-\w CJK character essentially never matches). Tagging at the
 * source keeps the guard scoped to what actually needs it.
 */
function isFallbackOrigin(pattern) {
  return !!pattern && pattern.origin === 'fallback';
}

/**
 * Match text against a pattern, requiring a whole-token (word-boundary) match for
 * fallback-origin patterns instead of plain substring/regex matching.
 *
 * Fallback tokens are single alphanumeric rule-basenames (e.g. 'skill', derived
 * from `.../SKILL.md`) and are prone to firing on a coincidental SUBSTRING of an
 * unrelated longer identifier — most commonly a `.claude/skills/` path segment
 * (AC4) — even after tool_calls scan-surface narrowing has ruled out the
 * structural-field case (AC1). Explicit keyword-map entries (any other pattern)
 * are untouched: their substring/regex semantics are deliberately curated and
 * already exercised by existing tests.
 * @param {string} text
 * @param {{kw, sev, kind, rule, raw, origin?}} pattern
 * @returns {boolean}
 */
function matchesWithFallbackGuard(text, pattern) {
  if (!isFallbackOrigin(pattern) || typeof pattern.kw !== 'string') {
    return matchPattern(text, pattern);
  }
  const re = fallbackWordRe(pattern.kw);
  return re ? re.test(text) : matchPattern(text, pattern);
}

/**
 * Compile-once cache for fallback word-boundary regexes. Without it a RegExp was
 * constructed per (pattern × scanned text) across all three scan loops. Keys are
 * sanitized rule basenames from buildFallbackPatterns(), bounded by --top-n hot
 * rules (default 5). No `g` flag → `.test()` stays stateless across calls.
 * [CR F6 · td-devenv-guard-gate-false-signal-repair]
 */
const FALLBACK_WORD_RE_CACHE = new Map();
function fallbackWordRe(kw) {
  if (FALLBACK_WORD_RE_CACHE.has(kw)) return FALLBACK_WORD_RE_CACHE.get(kw);
  let re;
  try {
    re = new RegExp(`\\b${kw.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`, 'i');
  } catch {
    re = null; // caller falls back to plain substring matching
  }
  FALLBACK_WORD_RE_CACHE.set(kw, re);
  return re;
}
...
export function scanSessionActions(actions, patterns) {
  ...
  for (let i = 0; i < messages.length; i++) {
    ...
    for (const p of patterns) {
      // Skip fallback-origin matches on noise messages to avoid the systematic
      // false-positive from Entry Gate acknowledgement lists etc.
      // [CR F3] Uses the same `origin` tag as matchesWithFallbackGuard() rather than
      // the old (sev==='low' && kind==='string') shape guess. That guess also swallowed
      // curated keyword-map entries sharing the shape — `.claude/rules/cr-debt-doc-audit.md`
      // really does define `"Boy Scout 跳過"` at sev=low/kind=string — silencing an
      // explicit keyword the map author intended to fire.
      if (noise && isFallbackOrigin(p)) continue;
      if (matchesWithFallbackGuard(text, p)) {
  ...
  for (let i = 0; i < toolCalls.length; i++) {
    ...
    const scanText = extractToolCallScanText(tc);
    if (!scanText) continue;
    for (const p of patterns) {
      if (matchesWithFallbackGuard(scanText, p)) {
        findings.push(buildFinding(p, `tool_call[${i}]`, scanText, tc?.name));
      }
    }
  }
  ...
  for (let i = 0; i < fileChanges.length; i++) {
    ...
    for (const p of patterns) {
      if (matchesWithFallbackGuard(serialized, p)) {
