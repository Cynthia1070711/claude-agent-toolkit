// ============================================================
// PhyCool Context Memory DB — Session 自動記錄腳本
...
//   - PreCompact Hook: context compaction 前（settings.json）
// 策略:
...
const PROJECT_FOLDER = '${PROJECT_SLUG}';
...
// ──────────────────────────────────────────────
// 從 stories 表提取 in-progress / review Story
// [tdb-4] 舊來源（Sprint 排程 yaml）已於 2026-07-28（tdb-2）凍結為唯讀歷史快照，
//   續讀該檔會讓每筆 session 記錄帶著停格的舊清單。DB 為 Story 狀態 SSoT。
//   existingDb 可選：生產路徑傳入既有連線（零額外開銷），測試／獨立呼叫時自開 readonly。
// ──────────────────────────────────────────────
function getActiveStories(existingDb = null) {
  let db = existingDb;
  try {
    if (!db) {
    ...
    const rows = db.prepare(
      `SELECT s.story_id, s.status FROM stories s
        LEFT JOIN track_plan tp ON tp.story_id = s.story_id
        WHERE s.status IN ('in-progress', 'review')
          AND (tp.plan_state IS NULL OR tp.plan_state != 'paused')
        ORDER BY s.story_id`
    ).all();
    return rows.map(r => ({ id: r.story_id, status: r.status }));
  } catch {
    return [];
  } finally {
    if (!existingDb && db) {
...
// ──────────────────────────────────────────────
// 組合 session 摘要內容
// CMI-5 增強：整合 Story DB 狀態 + assistant message 結構化提取
// ──────────────────────────────────────────────
function buildSessionContent(hookEvent, lastAssistantMessage, existingDb = null) {
  const activeStories = getActiveStories(existingDb);
...
async function writeSessionToDB() {
  ...
  try {
    ...
    const content = buildSessionContent(hookEvent, lastMsg, db);
    ...
    // TZ FIX (2026-05-29): cutoff 原 toISOString()=UTC(...Z),與 timestamp 欄(getTaiwanTimestamp 寫的 +08:00 字面戳)
    //   做 SQLite 字典序 `timestamp > ?` 比較偏 8h → 2 分鐘去重視窗失效。改台灣 +08:00 同基準。
    const cutoff = new Date(Date.now() - DEDUP_MINUTES * 60 * 1000).toLocaleString('sv-SE', { timeZone: 'Asia/Taipei' }).replace(' ', 'T') + '+08:00';
