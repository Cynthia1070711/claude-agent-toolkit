// ============================================================
// PhyCool Context Memory DB — 技術債寫入工具
...
async function upsertDebt(data, { validateTarget = false } = {}) {
  ...
  `).run({
    ...
    // 2026-07-26 whp-1 CR: ACCEPTED 分類的兩個必填欄位(saas-standards §Non-Fixed Issue
    // Classification / step-05 FK Gate missing-review-date)原本不在此 INSERT 內, 導致經本
    // 工具寫入的 ACCEPTED 一律 review_date=NULL(實測 45 筆 accepted 僅 8 筆有值, 那 8 筆來自
    // 一次性 migration/rescue script)。純加法, 未提供時為 null, 既有 300 處 CLI 呼叫不受影響。
    review_date: data.review_date || null,
    accepted_reason: data.accepted_reason || null,
  });
...
// [母卡收口 2026-08-04] 轉靶模式 —— 收口傘卡標 done 前,把 target_story 指向它的
// open debt 逐筆移到存活載體(對齊 tdb 母卡確立的收口新規,防 whp 母卡孤兒 debt 事故)。
// 鏡像 resolveDebt 形狀:單句 UPDATE 只動 target_story,絕不觸碰其他欄位(本檔的
// 完整 upsert 為 INSERT OR REPLACE,部分 payload 會清空未帶欄位,故轉靶禁走該路徑)。
function retargetDebt(debtId, toStory, byAgent) {
  const db = new Database(DB_PATH);
  db.pragma('journal_mode = WAL');

  const exists = db.prepare('SELECT story_id FROM stories WHERE story_id = ?').get(toStory);
  if (!exists) {
    db.close();
    console.error(`❌ target_story '${toStory}' 不存在於 stories 表`);
    process.exit(1);
  }
  ...
  if (result.changes > 0) {
    console.log(`✅ Debt ${debtId} 已轉靶 → ${toStory} (by: ${byAgent})`);
  } else {
    console.warn(`⚠️ Debt ${debtId} 未轉靶（不存在或已 fixed）`);
  }
}
...
// Export for testing
export { upsertDebt, resolveDebt, retargetDebt, queryDebts, showStats, DB_PATH };
...
if (args.length === 0) {
  ...
  console.log("  node upsert-debt.js --retarget <debt-id> --to <story> [--by <agent>]  # 轉靶(收口用,只動 target_story)");
...
(async () => {
  ...
  } else if (args[0] === '--retarget') {
    const debtId = args[1];
    const toIdx = args.indexOf('--to');
    const byIdx = args.indexOf('--by');
    const toStory = toIdx >= 0 ? args[toIdx + 1] : null;
    const byAgent = byIdx >= 0 ? args[byIdx + 1] : 'unknown';
    if (!debtId || !toStory) {
      console.error('❌ --retarget 需要 <debt-id> 與 --to <story>');
      process.exit(1);
    }
    retargetDebt(debtId, toStory, byAgent);
