// ============================================================
// PhyCool Context Memory DB — 單一 Story 寫入工具
...
import { normalizeMarkdownBreaks } from './markdown-normalize.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const DB_PATH = process.env.PHYCOOL_DB_PATH || path.join(__dirname, '..', 'phycool.db');

function getTaiwanTimestamp() {
  return new Date().toLocaleString('sv-SE', { timeZone: 'Asia/Taipei' }).replace(' ', 'T') + '+08:00';
}

// [tdb-1-track-plan-roadmap] track_plan auto-exit 終態 status 集合 —— 對齊
// .context-db/scripts/migrate-track-plan.js 的 TERMINAL_STATUSES(逐字同源,避免雙處定義漂移）。
const TRACK_PLAN_EXIT_STATUSES = new Set(['done', 'cancelled', 'cancelled-merged', 'superseded', 'split', 'skipped', 'deleted']);

/**
 * [tdb-1] track_plan auto-exit —— Story 進終態時同步 plan_state='done-exited'。
 * 三層保護：① 無 track_plan 列 → 靜默略過不 INSERT（未排程卡）
 *          ② 已是 done-exited → 不更新 updated_at（避免製造假的排程異動時間）
 *          ③ try/catch 降級 → track_plan 表不存在時不使 Story 寫入失敗
 *            （upsert-story.js 是全專案唯一 Story 寫入路徑，副作用必須可降級）。
 * 掛在 _doUpsert（非 mergeStory）—— mergeStory 最終回呼 _doUpsert，掛在 mergeStory 會漏完整 upsert 路徑。
 */
function autoExitTrackPlan(db, storyId, status) {
  if (!TRACK_PLAN_EXIT_STATUSES.has(status)) return;
  try {
    const row = db.prepare('SELECT plan_state FROM track_plan WHERE story_id = ?').get(storyId);
    if (!row) return;
    if (row.plan_state === 'done-exited') return;
    db.prepare(`UPDATE track_plan SET plan_state='done-exited', seq=NULL, updated_at=?, updated_by=? WHERE story_id=?`)
      .run(getTaiwanTimestamp(), 'upsert-story-auto-exit', storyId);
    if (!quietMode) {
      console.log(`🔁 track_plan auto-exit: ${storyId} → plan_state='done-exited' (status=${status})`);
    }
  } catch (err) {
    if (!quietMode) {
      console.warn(`⚠️  track_plan auto-exit 失敗（不影響 Story 寫入）: ${err.message}`);
    }
  }
}
...
const KNOWN_COLUMNS = new Set([
  ...
  'tasks', 'cr_score', 'test_count', 'discovery_source', 'updated_at',
  'affected_files', // DEPRECATED (bwu-1): 無 workflow 讀寫者，schema 保留不刪，僅 tech_debt_items 同名欄有用
  ...
  'execution_log', // DEPRECATED (bwu-1): 無 workflow 讀寫者，schema 保留不刪
  'sdd_spec', 'create_agent', 'create_started_at', 'create_completed_at',
  'pipeline_notes', 'review_started_at',
  // party-to-pipeline v4.0.0 主線/副線分流 (init-db.js v2.3 schema)
  'task_track'
]);

// [1A · 2026-06-12] Markdown 換行 normalize（DevConsole react-markdown 渲染防護）
// 背景: DevConsole StoryDetail.tsx 用 react-markdown 渲染 Story 欄位,單 \n = soft break(渲染成空格·同段落),
//   worker/agent 寫 enrichment 項目憑直覺用單 \n（terminal 看似換行）→ DevConsole 渲染「全部混在一起沒換行」
//   (2026-06-12 m0-11 事故)。修正: list marker / heading / ordered-list 前確保 \n\n 段落分隔。
//   所有寫入路徑經 _doUpsert 單點強制,不靠 agent 記性。
const MARKDOWN_BREAK_FIELDS = [
  'tasks', 'dev_notes', 'pipeline_notes', 'acceptance_criteria',
  'implementation_approach', 'testing_strategy', 'definition_of_done',
  'risk_assessment', 'rollback_plan', 'background',
];

function applyMarkdownNormalization(data) {
  for (const f of MARKDOWN_BREAK_FIELDS) {
    if (typeof data[f] === 'string') data[f] = normalizeMarkdownBreaks(data[f]);
  }
}
...
async function _doUpsert(db, data) {
  ...
  // [1A] Markdown 換行 normalize（寫入層單點強制 — 完整 upsert 與 merge 皆經此 _doUpsert）
  applyMarkdownNormalization(data);
  ...
  db.prepare(`
    INSERT OR REPLACE INTO stories
      (story_id, epic_id, domain, title, status, priority, complexity,
       ...
       sdd_spec, create_agent, create_started_at, create_completed_at,
       pipeline_notes, review_started_at, task_track)
    VALUES
      (@story_id, @epic_id, @domain, @title, @status, @priority, @complexity,
       @story_type, @dependencies, @tags, @file_list, @dev_agent, @review_agent,
       @source_file, @created_at, @user_story, @background, @acceptance_criteria,
  ...
  `).run({
    ...
    task_track: data.task_track || 'main',
  });
  ...
  // [tdb-1] track_plan auto-exit — 最終 status 進終態時同步排程列退出（見上方函式註解）
  autoExitTrackPlan(db, data.story_id, data.status || 'backlog');
...
async function mergeStory(storyId, updates, existingDb = null) {
  ...
  if (!('status' in updates)) {
    const AUTO_PROMOTE_RULES = [
      ...
      // [2026-07-29 hotfix] I2 enforcement gap: started_at(dev 開工)原本無對應推進條目,
      // dev 全程停留 ready-for-dev → DevConsole /stories 看板「開發中」區恆空(使用者實測回報)。
      // 刻意置於最後: 同一 merge 同帶更終態 trigger(completed_at / review_completed_at)時
      // 由前面條目先命中 break,終態優先不受本條影響。
      { trigger: 'started_at',          from: ['ready-for-dev'],                          to: 'in-progress' },
    ];
...
// [tdb-1-track-plan-roadmap] isMain guard —— 讓本檔可安全 import 供測試使用
// （原檔底部 CLI 區塊無條件執行,import 會立即對 process.argv 跑 CLI 邏輯甚至 process.exit;
//   對齊 migrate-track-plan.js 既有寫法,upsert-story-normalize-wiring.test.js 註解記載此為既定升級路徑）。
...
// [tdb-1-track-plan-roadmap] Exports —— 供 .context-db/tests/*.test.js 直接 import 驅動
// （mergeStory 既有 existingDb 參數已支援注入 temp DB;autoExitTrackPlan 同樣以 db 為首參數）。
export {
  upsertStory, mergeStory, _doUpsert, autoExitTrackPlan,
  TRACK_PLAN_EXIT_STATUSES, KNOWN_COLUMNS, applyMarkdownNormalization,
  getTaiwanTimestamp,
};
