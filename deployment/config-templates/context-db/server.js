// ============================================================
// PhyCool Context Memory DB — MCP Server
...
import { searchWorkerRuns, ackWorkerRun, gateWorkerRun, addWorkerMessage } from './scripts/worker-protocol-ops.js';
import { postCtrlMessage, readCtrlMessages, closeCtrlThread, updateCtrlBoard } from './scripts/ctrl-channel-ops.js';
...
function appendLedger(table, operation, data) {
  try {
    ...
    process.stderr.write(`[phycool-context] ledger append error: ${err.message}\n`);
  }
}
...
async function getVectorScores(database, tableName, queryText, topK = 50) {
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] vector search fallback (${tableName}): ${err.message}\n`);
    return new Map();
  }
}
...
function fuseResults(ftsRows, vectorScores, idExtractor, limit) {
  ...
  if (ftsRows.length === 0 && vectorScores.size > 0) {
    process.stderr.write(`[phycool-context] hybrid: FTS=0, vector=${vectorScores.size} — rerank 模式無法補充 vector-only 結果\n`);
    return [];
  }
...
const server = new Server(
  { name: 'phycool-context', version: '0.2.0' },
  { capabilities: { tools: {} } }
);
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
        ...
        properties: {
          ...
          filters: {
            ...
            properties: {
              ...
              date_from: { type: 'string', description: '起始日期過濾（ISO 如 2026-05-01，或自然語言「昨天」「上週」「本月」「近七天」等 zh-Hant · Asia/Taipei UTC+8）。比對 timestamp >=。' },
              date_to: { type: 'string', description: '結束日期過濾（ISO 或自然語言，自動取當天結束 23:59:59）。比對 timestamp <=。' },
            },
          },
        },
        required: ['query'],
    ...
    // ── Phase 1: Symbol 級程式碼檢索（TD-33）──
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          namespace: {
            type: 'string',
            description: '命名空間前綴過濾（選填，e.g., "PhyCool.Web.Services"）',
          },
          limit: {
            type: 'number',
    ...
    // ── ADR-GOVERNANCE-001: God Node 識別工具(Tianji-Pavilion v1.1.0 + capability-integration-mandate.md)──
    {
      name: 'search_god_nodes',
      description: '查詢高 centrality 的 god node symbol(被廣泛依賴的核心 service / entity)。資料來自 god_nodes 表(weighted PageRank, RELATION_WEIGHTS) + symbol_index JOIN。包含 blast_radius(critical/high/medium/low by P95/P75/P50)+ computed_at 時間戳。預設排除 is_generated/is_test=1 的符號。用於 dev-story 定位核心 + CR BlastRadius 評估 + cross-Story refactor 影響分析。',
      inputSchema: {
        type: 'object',
        properties: {
          domain: {
            type: 'string',
            description: 'namespace 過濾(選填,e.g., "Payment" / "BackOffice")— LIKE 模糊匹配 si.namespace',
          },
          limit: {
            type: 'number',
          ...
          include_generated: {
            type: 'boolean',
            description: '是否包含 generated code(Migrations/ModelSnapshot/Tests)(預設 false)',
          },
          blast_radius: {
            type: 'string',
            description: 'blast_radius 過濾(選填): "critical" | "high" | "medium" | "low"。critical=P95+ / high=P75-P95 / medium=P50-P75 / low<P50',
          },
        },
      },
    },
    ...
    {
      name: 'search_stories',
      description: '查詢 Story 索引（stories 表 47 欄位）。支援 FTS5 全文搜尋（title/tags/dependencies）+ 多條件過濾。空 query 回傳最近 Story。用於查詢 Story 狀態、Epic 進度、開發追蹤、CR 統計等。',
      inputSchema: {
        type: 'object',
        properties: {
          query: {
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          related_skills:       { type: 'string', description: '相關 Skills（JSON array，e.g., ["phycool-editor-arch"]）' },
    ...
    // ── ECC-07: Instinct Management Tools (ecc-instincts registry) ──
    {
      name: 'add_instinct',
      description: '新增或更新 ECC Instinct 記錄。search-first (UNIQUE trigger+scope) 後 INSERT OR REPLACE。同步寫入 instincts 表 + Ledger dual-write。',
      inputSchema: {
        type: 'object',
        properties: {
          trigger:          { type: 'string', description: '觸發條件描述（唯一識別鍵之一，與 scope 組成 UNIQUE 約束）' },
          action:           { type: 'string', description: '應執行的動作描述' },
          confidence:       { type: 'number', description: 'Instinct 可信度 0.0~1.0（必填）' },
          scope:            { type: 'string', description: 'Instinct 範疇: "session" | "pipeline-session" | "project" | "global"' },
          domain:           { type: 'string', description: '領域分類（選填，e.g., "editor", "payment", "auth"）' },
          source:           { type: 'string', description: '來源描述（選填，e.g., "code-review", "incident-2026-05"）' },
          source_session_id:{ type: 'string', description: '來源 session ID（選填）' },
          evidence_jsonb:   { type: 'string', description: '支持佐證（JSON array 字串，選填）' },
          verifier_status:  { type: 'string', description: '驗證狀態: "approved" | "rejected" | "needs-more-evidence"（選填）' },
          verifier_reason:  { type: 'string', description: '驗證說明（選填）' },
        },
        required: ['trigger', 'action', 'confidence', 'scope'],
      },
    },
    {
      name: 'search_instincts',
      description: '搜尋 ECC Instinct 列表。支援 scope/confidence/domain/verifier_status 過濾 + trigger/action LIKE 查詢。ORDER BY confidence DESC, last_seen DESC。',
      inputSchema: {
        type: 'object',
        properties: {
          query:           { type: 'string', description: '關鍵字查詢（對 trigger + action 進行 LIKE 比對，空字串回傳全部）' },
          scope:           { type: 'string', description: 'scope 過濾: "session" | "pipeline-session" | "project" | "global"' },
          min_confidence:  { type: 'number', description: '最低 confidence 門檻（0.0~1.0）' },
          domain:          { type: 'string', description: 'domain 過濾（精確比對）' },
          verifier_status: { type: 'string', description: 'verifier_status 過濾: "approved" | "rejected" | "needs-more-evidence"' },
          limit:           { type: 'number', description: '回傳筆數上限（預設 20，最大 100）' },
        },
        required: [],
      },
    },
    {
      name: 'decay_instinct',
      description: '對指定 Instinct 執行 confidence 衰減（預設 -0.1）。若衰減後 confidence < 0.3 則設定 decay_at 時間戳。',
      inputSchema: {
        type: 'object',
        properties: {
          id:     { type: 'string', description: 'Instinct ID（必填）' },
          amount: { type: 'number', description: '衰減量（預設 0.1，正數）' },
        },
        required: ['id'],
      },
    },
    {
      name: 'promote_instinct',
      description: '將符合條件的 Instinct 晉升至 global scope。條件：confidence >= 0.8 AND verifier_status = "approved" AND scope = "project"。',
      inputSchema: {
        type: 'object',
        properties: {
          id: { type: 'string', description: 'Instinct ID（必填）' },
    ...
    // whp-5-message-bus-mcp: 4 個 worker protocol 訊息匯流排 + 執行權 CAS tool
    {
      name: 'search_worker_runs',
      description: '查詢 worker_runs 派發歷程 / 待簽收佇列（pending_ack）/ 殭屍清單。唯讀，不寫入任何表，不計入 SEARCH_TOOLS 檢索觀測（worker_runs 為運行狀態非可檢索知識）。',
      inputSchema: {
        type: 'object',
        properties: {
          story_id:           { type: 'string', description: 'Story ID 過濾（精確比對）' },
          phase:               { type: 'string', description: 'Phase 過濾（精確比對，e.g. "dev-story"）' },
          lifecycle:           { type: 'string', description: 'lifecycle 過濾，逗號分隔多值走 IN（e.g. "reported,awaiting-review"）' },
          controller_track:    { type: 'string', description: '中控軌道過濾（精確比對）' },
          run_mode:            { type: 'string', description: 'run_mode 過濾: "window" | "inline"' },
          requires_attention:  { type: 'boolean', description: '過濾 requires_attention 旗標' },
          pending_ack:         { type: 'boolean', description: '待簽收佇列（lifecycle=reported AND ack_at IS NULL），依 reported_at ASC 排序（最久未簽收排最前）。與 lifecycle 參數互斥 — 指定 pending_ack 時 lifecycle 過濾不生效（lifecycle 已固定為 reported）' },
          include_messages:    { type: 'boolean', description: '是否附帶 worker_messages（單次批次查詢，依 seq ASC）' },
          limit:               { type: 'number', description: '回傳筆數上限（預設 20，硬上限 100）' },
        },
        required: [],
      },
    },
    {
      name: 'ack_worker_run',
      description: '中控對 lifecycle=reported 的 run 簽收（CAS）。簽收不是裁決，不觸碰 worker_handoffs。重複簽收回傳 {ok:false} 且不設 isError。',
      inputSchema: {
        type: 'object',
        properties: {
          run_id: { type: 'string', description: 'worker_runs.run_id（必填）' },
          ack_by: { type: 'string', description: '簽收者 Agent ID，不得為空字串（必填）' },
        },
        required: ['run_id', 'ack_by'],
      },
    },
    {
      name: 'gate_worker_run',
      description: '中控對已簽收（ack_at 非 NULL）且 lifecycle=awaiting-review 的 run 下裁決。裁決 + lifecycle 推進 + 指示訊息三者同一 transaction 原子完成。跨軌裁決須 override + 非空 override_reason。',
      inputSchema: {
        type: 'object',
        properties: {
          run_id:           { type: 'string', description: 'worker_runs.run_id（必填）' },
          verdict:          { type: 'string', description: '裁決結果: "approved" | "revise" | "rejected"（必填）' },
          gate_by:          { type: 'string', description: '裁決者 Agent ID（必填）' },
          gate_notes:       { type: 'string', description: '裁決備註；verdict="revise" 時為必填，將成為指示訊息 body' },
          next_phase:       { type: 'string', description: '下一階段（選填）' },
          controller_track: { type: 'string', description: '呼叫端所屬軌道（選填，預設沿用 run 自身 controller_track）' },
          override:         { type: 'boolean', description: '跨軌裁決旗標（選填）' },
          override_reason:  { type: 'string', description: '跨軌裁決理由，override=true 時須非空（選填）' },
        },
        required: ['run_id', 'verdict', 'gate_by'],
      },
    },
    {
      name: 'add_worker_message',
      description: '中控發指示 / 喚醒 / 回覆至 worker_messages。mode 無預設值，必須明示 "append" 或 "replace"。msg_type 依 direction 各自驗證（非聯集）。永不寫 delivered/consumed（屬 whp-6/whp-11 送達通道職責）。',
      inputSchema: {
        type: 'object',
        properties: {
          run_id:    { type: 'string', description: 'worker_runs.run_id（必填）' },
          msg_type:  { type: 'string', description: '訊息類型，依 direction 各自合法值（必填）' },
          body:      { type: 'string', description: '訊息內容（必填）' },
          author:    { type: 'string', description: '發送者 Agent ID（必填）' },
          mode:      { type: 'string', description: '"append"（追加）或 "replace"（取代既有 pending controller-to-worker 訊息），無預設值，必填' },
          direction: { type: 'string', description: '"controller-to-worker"（預設）或 "worker-to-controller"（選填）' },
        },
        required: ['run_id', 'msg_type', 'body', 'author', 'mode'],
      },
    },
    // ccb-1-db-mcp-import: 4 個聊天室通道 tool(唯一寫入路徑,取代 2 個 markdown 聊天室 + 15 封存檔)
    {
      name: 'post_ctrl_message',
      description: '發訊息至 ctrl_ 聊天室通道;未帶 thread_id 時自動建立新話題(流水號格式)。closed thread 拒發(CAS,非 isError)。superseded_by 僅能標記呼叫端自己 from_track 的既有訊息。',
      inputSchema: {
        type: 'object',
        properties: {
          thread_id:     { type: 'string', description: '既有話題 ID(選填,省略即開新題,格式 "YYYY-MM-DD HH:mm:ss")' },
          topic:         { type: 'string', description: '話題標題(省略 thread_id 開新題時必填)' },
          category:      { type: 'string', description: '話題分類(選填)' },
          channel:       { type: 'string', description: '頻道:"general" | "correction" | 未來新值(選填,預設 "general")' },
          must_read:     { type: 'boolean', description: '是否標記必讀(選填,預設 false,僅開新題時生效)' },
          from_track:    { type: 'string', description: '發送軌別(選填,省略時取環境變數 PHYCOOL_CONTROLLER_TRACK)' },
          to_tracks:     { type: 'array', items: { type: 'string' }, description: '收件軌別清單(必填,非空陣列,顯式收件無預設值)' },
          msg_type:      { type: 'string', description: '"inform" | "discuss" | "request" | "handoff" | "decision" | "state"(必填)' },
          body:          { type: 'string', description: '訊息內容(必填)' },
          ref_json:      { type: 'object', description: '結構化參照(選填,如 story_id/commit/file/board_id/thread_id 等)' },
          superseded_by: { type: 'number', description: '更正鏈:標記本軌既有訊息(msg_id)為已被本則取代(選填,僅限本軌自己的訊息)' },
        },
        required: ['to_tracks', 'msg_type', 'body'],
      },
    },
    {
      name: 'read_ctrl_messages',
      description: '讀取 ctrl_ 聊天室通道訊息(兼 list / 未讀 / 依題 / 依分類 / 依頻道),讀取即自動簽收(UPSERT ctrl_message_reads)。不做全文搜尋(歸 DevConsole /channel UI API)、不做 reopen。',
      inputSchema: {
        type: 'object',
        properties: {
          reader_track:    { type: 'string', description: '簽收軌別(選填,省略時取環境變數 PHYCOOL_CONTROLLER_TRACK)' },
          thread_id:       { type: 'string', description: '限定單一話題(選填)' },
          channel:         { type: 'string', description: '依頻道過濾(選填)' },
          category:        { type: 'string', description: '依分類過濾(選填)' },
          state:           { type: 'string', description: '依話題狀態過濾:"open" | "closed"(選填)' },
          unread_only:     { type: 'boolean', description: '只回傳本軌尚未簽收的訊息(選填)' },
          must_read_only:  { type: 'boolean', description: '只回傳話題標記 must_read 的訊息(選填)' },
          limit:           { type: 'number', description: '回傳筆數上限(選填,預設 20,硬上限 100,只對實際回傳的訊息簽收)' },
        },
        required: [],
      },
    },
    {
      name: 'close_ctrl_thread',
      description: '關閉話題(CAS,單句 UPDATE)。僅發起軌(initiator_track)可關閉,非發起軌或重複關閉皆回 {ok:false} 且不設 isError,並附當前 state/initiator_track。不提供任何 reopen 路徑(D2 裁定,續談請開新題)。',
      inputSchema: {
        type: 'object',
        properties: {
          thread_id:    { type: 'string', description: '欲關閉的話題 ID(必填)' },
          caller_track: { type: 'string', description: '呼叫端軌別(選填,省略時取環境變數 PHYCOOL_CONTROLLER_TRACK)' },
        },
        required: ['thread_id'],
      },
    },
    {
      name: 'update_ctrl_board',
      description: '更新共享看板(CAS 樂觀鎖,version+1)。state 須符合 BoardState shape(status/holder/history 必填,until/note 選填),不符合回 CCB1-E02。expected_version 不符回 {ok:false} 並附最新完整 state 供重讀重試;board_id 不存在回 CCB1-E03(不隱式建列)。',
      inputSchema: {
        type: 'object',
        properties: {
          board_id:         { type: 'string', description: '看板 ID,如 "staging-db"(必填)' },
          expected_version: { type: 'number', description: '樂觀鎖版本號,須為呼叫端讀到的最新 version(必填)' },
          state:            { type: 'object', description: 'BoardState:{status, holder, until?, note?, history:[{ts,track,action}]}(必填)' },
          updated_by:       { type: 'string', description: '更新者軌別(選填,省略時取環境變數 PHYCOOL_CONTROLLER_TRACK)' },
        },
        required: ['board_id', 'expected_version', 'state'],
      },
    },
  ],
}));
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  const SEARCH_TOOLS = new Set([
    ...
    'search_instincts',
    // 刻意不含 search_worker_runs：worker_runs 是運行狀態非可檢索知識（同判準見
    // upsert-worker-run.js 不接 embedding-sync.js）；且下方 entryId 抽取鏈用
    // r.id ?? r.story_id ?? r.session_id，worker_runs 每列皆有 session_id 但主鍵是
    // run_id，加入會記錯 id（whp-5-message-bus-mcp BR-006）。
    // 刻意不含 read_ctrl_messages（ccb-1-db-mcp-import T7 決策）：(1) 回傳形狀
    // {messages,count,signed} 不符下方任一解析分支（非陣列 / 無 .results / 無
    // .total），加入會靜默記 resultCount=0 + hitEntries=[]，與 search_worker_runs
    // 被排除的「id 欄位記錯」同一類「形狀不符」風險；(2) ctrl_messages 是跨軌協調
    // 訊息（運行時通訊）非長期可檢索知識庫，且本 tool 每次呼叫必產生簽收寫入副作用
    // （UPSERT ctrl_message_reads），不符合 SEARCH_TOOLS 隱含「唯讀」前提。
  ]);

  try {
    process.stderr.write(`[phycool-context] Tool call: ${name}\n`);
    ...
    switch (name) {
      ...
      case 'verify_intentional_annotations':
        ...
        break;
      // ECC-07: Instinct Management Tools
      case 'add_instinct':
        return handleAddInstinct(args);
      case 'search_instincts':
        result = handleSearchInstincts(args);
        break;
      case 'decay_instinct':
        return handleDecayInstinct(args);
      case 'promote_instinct':
        return handlePromoteInstinct(args);
      // whp-5-message-bus-mcp: 4 個 worker protocol tool
      case 'search_worker_runs':
        return handleSearchWorkerRuns(args);
      case 'ack_worker_run':
        return handleAckWorkerRun(args);
      case 'gate_worker_run':
        return handleGateWorkerRun(args);
      case 'add_worker_message':
        return handleAddWorkerMessage(args);
      // ccb-1-db-mcp-import: 4 個聊天室通道 tool
      case 'post_ctrl_message':
        return handlePostCtrlMessage(args);
      case 'read_ctrl_messages':
        return handleReadCtrlMessages(args);
      case 'close_ctrl_thread':
        return handleCloseCtrlThread(args);
      case 'update_ctrl_board':
        return handleUpdateCtrlBoard(args);
      default:
        process.stderr.write(`[phycool-context] Unknown tool requested: ${name}\n`);
        return {
          isError: true,
    ...
    process.stderr.write(`[phycool-context] Tool error (${name}): ${err.message}\n`);
    return {
      isError: true,
...
// search_context 實作
// ──────────────────────────────────────────────
// γ-6 (ADR-KHOJ-CHUNKING-001 §C DateFilter 蒸餾): 自然語言日期 → ISO+08:00 (Asia/Taipei)
// 對齊 constitutional-standard §Timestamp UTC+8。ISO 字串原樣返回; 無法解析則原樣 (容錯)。
export function parseDateExpr(expr, isEnd = false) {
  if (!expr) return null;
  const s = String(expr).trim();
  if (/^\d{4}-\d{2}-\d{2}/.test(s)) return s; // 已是 ISO 日期 → 原樣
  const tw = new Date(new Date().toLocaleString('en-US', { timeZone: 'Asia/Taipei' }));
  tw.setHours(0, 0, 0, 0);
  const shift = (days) => { const d = new Date(tw); d.setDate(d.getDate() + days); return d; };
  const dow = (tw.getDay() + 6) % 7; // 0=Mon .. 6=Sun (台灣以週一為週起點)
  let d;
  switch (s) {
    case '今天': case '今日': d = new Date(tw); break;
    case '昨天': case '昨日': d = shift(-1); break;
    case '前天': d = shift(-2); break;
    case '本週': case '本周': case '這週': case '這周': case '本禮拜': d = shift(-dow); break;
    case '上週': case '上周': case '上禮拜': case '上星期': d = shift(-dow - 7); break;
    case '本月': case '這個月': d = new Date(tw.getFullYear(), tw.getMonth(), 1); break;
    case '上月': case '上個月': d = new Date(tw.getFullYear(), tw.getMonth() - 1, 1); break;
    case '近七天': case '最近七天': case '過去七天': case '近 7 天': d = shift(-7); break;
    case '近三十天': case '最近三十天': case '過去三十天': case '近 30 天': d = shift(-30); break;
    default: return s; // 無法解析 → 原樣 (容錯)
  }
  if (isEnd) d.setHours(23, 59, 59, 0); // date_to: 當天結束
...
async function handleSearchContext(args) {
  ...
  // γ-6: 自然語言/ISO 日期 → ISO+08:00 (對齊 context_entries.timestamp 格式)
  const dateFrom = parseDateExpr(date_from, false);
  const dateTo = parseDateExpr(date_to, true);
  ...
  try {
    ...
    if (!query || query.trim() === '') {
      ...
      if (dateFrom) { sql += ' AND timestamp >= ?'; params.push(dateFrom); } // γ-6 日期過濾
      if (dateTo)   { sql += ' AND timestamp <= ?'; params.push(dateTo); }   // γ-6 日期過濾
      ...
      if (dateFrom) { sql += ' AND ce.timestamp >= ?'; params.push(dateFrom); } // γ-6 日期過濾
      if (dateTo)   { sql += ' AND ce.timestamp <= ?'; params.push(dateTo); }   // γ-6 日期過濾
    ...
    // FTS5 語法錯誤等
    process.stderr.write(`[phycool-context] search_context error: ${err.message}\n`);
    return {
      isError: true,
      content: [{
        type: 'text',
...
async function handleSearchTech(args) {
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] search_tech error: ${err.message}\n`);
    return {
      isError: true,
      content: [{
        type: 'text',
...
function handleAddContext(args) {
  ...
  try {
    ...
    syncEmbedding(database, 'context', entryId, embText)
      .catch(err => process.stderr.write(`[phycool-context] context embedding sync error (id=${entryId}): ${err.message}\n`));
    ...
    process.stderr.write(`[phycool-context] add_context error: ${err.message}\n`);
...
function handleAddTech(args) {
  ...
  try {
    ...
    syncEmbedding(database, 'tech', techEntryId, techEmbText)
      .catch(err => process.stderr.write(`[phycool-context] tech embedding sync error (id=${techEntryId}): ${err.message}\n`));
    ...
    process.stderr.write(`[phycool-context] add_tech error: ${err.message}\n`);
...
function handleAddCrIssue(args) {
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] add_cr_issue error: ${err.message}\n`);
...
function handleTraceContext(args) {
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] trace_context error: ${err.message}\n`);
...
function handleSearchSymbols(args) {
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] search_symbols error: ${err.message}\n`);
...
function handleGetSymbolContext(args) {
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] get_symbol_context error: ${err.message}\n`);
    return { isError: true, content: [{ type: 'text', text: `get_symbol_context 失敗：${err.message}` }] };
  }
}

// ──────────────────────────────────────────────
// search_god_nodes 實作 (ADR-GOVERNANCE-001 + Tianji-Pavilion v1.1.0)
...
function handleSearchGodNodes(args) {
  ...
  try {
    // Check if god_nodes table exists and has data (td-god-nodes-table-rebuild)
    const godNodesTable = database.prepare(
      `SELECT name FROM sqlite_master WHERE type='table' AND name='god_nodes'`
    ).get();

    const godNodesCount = godNodesTable
      ? database.prepare(`SELECT COUNT(*) AS c FROM god_nodes`).get().c
      : 0;

    if (!godNodesTable || godNodesCount === 0) {
      // Backwards-compat fallback: legacy symbol_index.centrality_score query
      process.stderr.write(
        '[search_god_nodes] god_nodes empty, falling back to legacy column. Run compute-centrality.cjs.\n'
      );
      ...
      if (!cols.find(c => c.name === 'centrality_score')) {
        return {
          isError: true,
          content: [{ type: 'text', text: 'god_nodes table empty AND symbol_index.centrality_score missing. Run migration + compute-centrality.cjs.' }],
        };
      }
      ...
      if (!include_generated) {
        fallbackSql += ` AND namespace NOT LIKE '%Migrations%' AND namespace NOT LIKE '%ModelSnapshot%' AND namespace NOT LIKE '%Tests%'`;
      }
      if (domain) { fallbackSql += ` AND namespace LIKE ?`; fallbackParams.push(`%${domain}%`); }
      if (min_centrality !== null) { fallbackSql += ` AND centrality_score >= ?`; fallbackParams.push(Number(min_centrality)); }
      fallbackSql += ` ORDER BY centrality_score DESC LIMIT ?`;
      fallbackParams.push(safeLimit);

      const fallbackRows = database.prepare(fallbackSql).all(...fallbackParams);
      return {
        content: [{
          type: 'text', text: JSON.stringify({
            total: fallbackRows.length,
            source: 'legacy_fallback',
            hint: 'god_nodes table empty — showing legacy symbol_index.centrality_score. Run: node .context-db/scripts/compute-centrality.cjs',
            god_nodes: fallbackRows.map(r => ({
              id: r.id, symbol_name: r.symbol_name, full_name: r.full_name,
              namespace: r.namespace, file_path: r.file_path, symbol_type: r.symbol_type,
    ...
    // Primary path: query god_nodes JOIN symbol_index
    const latestRow = database.prepare(`SELECT MAX(computed_at) AS ts FROM god_nodes`).get();
    const computedAt = latestRow ? latestRow.ts : null;

    let isStale = false;
    if (computedAt) {
      // Preserve +08:00 offset (don't replace with 'Z') so Taiwan time is parsed correctly.
      const tsMs = new Date(computedAt.replace(' ', 'T')).getTime();
      isStale = (Date.now() - tsMs) > 7 * 24 * 60 * 60 * 1000;
    }

    let sql = `
      SELECT gn.symbol_id, gn.centrality_score, gn.blast_radius, gn.in_degree, gn.out_degree,
             gn.weighted_in_degree, gn.weighted_out_degree, gn.is_generated, gn.is_test,
             gn.domain, gn.computed_at,
             si.symbol_name, si.full_name, si.namespace, si.symbol_type,
             si.file_path AS si_file_path, si.start_line, si.end_line, si.signature
      FROM god_nodes gn
      JOIN symbol_index si ON gn.symbol_id = si.id
      WHERE gn.centrality_score > 0
    `;
    const params = [];

    if (!include_generated) {
      sql += ` AND gn.is_generated = 0 AND gn.is_test = 0`;
    }

    if (blast_radius) {
      sql += ` AND gn.blast_radius = ?`;
      params.push(blast_radius);
    }

    if (domain) {
      sql += ` AND si.namespace LIKE ?`;
      params.push(`%${domain}%`);
    }

    if (min_centrality !== null && min_centrality !== undefined) {
      sql += ` AND gn.centrality_score >= ?`;
      params.push(Number(min_centrality));
    }

    sql += ` ORDER BY gn.centrality_score DESC LIMIT ?`;
    params.push(safeLimit);
    ...
    const result = {
      ...
      is_stale: isStale,
      staleness_hint: isStale
        ? `⚠ god_nodes data may be stale (last computed ${computedAt}). Run: node .context-db/scripts/compute-centrality.cjs --force`
        : null,
      filter: {
        domain: domain || null,
        blast_radius: blast_radius || null,
      ...
      god_nodes: rows.map(r => ({
        ...
        weighted_in_degree: +Number(r.weighted_in_degree).toFixed(4),
        weighted_out_degree: +Number(r.weighted_out_degree).toFixed(4),
        is_generated: !!r.is_generated,
        is_test: !!r.is_test,
      })),
      hint: '高 centrality_score = 被多個 symbol 依賴(weighted PageRank)。blast_radius: critical=P95 / high=P75 / medium=P50 / low。修改前必先 Read + 評估 blast_radius。詳細 dependency 用 get_symbol_context(symbol_id, depth=2)展開。',
    };
    ...
    process.stderr.write(`[phycool-context] search_god_nodes error: ${err.message}\n`);
...
async function handleSearchConversations(args) {
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] search_conversations error: ${err.message}\n`);
    return {
      isError: true,
      content: [{
        type: 'text',
...
function handleGetSessionDetail(args) {
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] get_session_detail error: ${err.message}\n`);
...
function handleListSessions(args) {
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] list_sessions error: ${err.message}\n`);
...
async function handleSemanticSearch(args) {
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] semantic_search: query="${query.trim()}", candidates=${rows.length}, passed_threshold=${scored.length}, returned=${topN.length}\n`);
    ...
    process.stderr.write(`[phycool-context] semantic_search vector error, falling back to LIKE: ${vectorErr.message}\n`);
    ...
    try {
      ...
      process.stderr.write(`[phycool-context] semantic_search fallback also failed: ${fallbackErr.message}\n`);
      return {
        isError: true,
        content: [{
          type: 'text',
...
async function handleSearchDocuments(args) {
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] search_documents FTS5 error: ${ftsErr.message}\n`);
  }
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] search_documents: query="${trimmedQuery}", vec_candidates=${rows.length}, vec_passed=${vectorResults.length}\n`);

  } catch (err) {
    vectorErr = err;
    degraded = true;
    process.stderr.write(`[phycool-context] search_documents vector fallback: ${err.message}\n`);
  }
...
async function handleSearchStories(args) {
  ...
  // bwu-1: 46 = 47(schema) - 1，刻意不含 affected_files。注意此排除僅限本白名單（`fields` 輕量查詢路徑）——
  // `detailCols`（下方 include_details:true 精確查詢路徑）仍選取 s.affected_files 未動，該欄位透過此路徑仍可讀取。
  // 「deprecated」意指 BMAD workflow 端無讀寫者（見 upsert-story.js KNOWN_COLUMNS 註解），非 MCP 層不可達。
  // 若未來新增 stories 欄位，白名單需同步 +1；請勿把 affected_files「補」回本白名單。
  const ALLOWED_STORY_FIELDS = new Set([
    ...
    'pipeline_notes', 'review_started_at', 'risk_assessment', 'task_track',
  ]);
  ...
  try {
    ...
    if (fields) {
      ...
      const askedFields = fields.split(',').map(f => f.trim()).filter(Boolean);
      const invalidFields = askedFields.filter(f => !ALLOWED_STORY_FIELDS.has(f));
      // bwu-1 CR-R2 (F1): 任何無效欄位一律顯式報錯。舊行為只在「全部無效」時報錯，
      // 混合請求（valid + invalid）會靜默丟掉無效欄位而呼叫端無從察覺 —— 這正是 G-8 的
      // 傷害機制本身（task_track 即以此方式靜默消失，見 stories.dev_notes D-6 活體證據）。
      if (askedFields.length === 0 || invalidFields.length > 0) {
        const detail = invalidFields.length > 0 ? `含無效欄位：${invalidFields.join(', ')}` : '無有效欄位';
        return { isError: true, content: [{ type: 'text', text: `fields 參數${detail}。允許欄位：${[...ALLOWED_STORY_FIELDS].join(', ')}` }] };
      }
      selectCols = askedFields.map(f => `s.${f}`).join(', ');
    } else {
      // 預設模式：摘要 + 可選詳情（向後相容）
      ...
      if (isPreciseStoryQuery && include_details) {
        // 精確查詢 + 詳情:回傳完整(無 _preview, 涵蓋全 stories 表 47 欄位)
        ...
        // review_started_at / source_file / affected_files / cr_summary / task_track 全可取
        detailCols = `,
    ...
    process.stderr.write(`[phycool-context] search_stories error: ${err.message}\n`);
    return {
      isError: true,
...
async function handleSearchDebt(args) {
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] search_debt error: ${err.message}\n`);
    return {
      isError: true,
...
function handleSearchGlossary(args) {
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] search_glossary error: ${err.message}\n`);
...
function handleLogWorkflow(args) {
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] log_workflow error: ${err.message}\n`);
...
function handleUpsertBenchmark(args) {
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] upsert_benchmark error: ${err.message}\n`);
...
function handleGetPatterns(args) {
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] get_patterns error: ${err.message}\n`);
...
function handleSearchIntentionalDecisions(args) {
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] search_intentional_decisions error: ${err.message}\n`);
...
function handleGetIntentionalDecision(args) {
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] get_intentional_decision error: ${err.message}\n`);
...
function handleAddIntentionalDecision(args) {
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] add_intentional_decision error: ${err.message}\n`);
...
function handleVerifyIntentionalAnnotations(args) {
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] verify_intentional_annotations error: ${err.message}\n`);
    return { isError: true, content: [{ type: 'text', text: `verify_intentional_annotations 失敗：${err.message}` }] };
  }
}

// ──────────────────────────────────────────────
// ECC-07: Instinct Management Tool Handlers
// ──────────────────────────────────────────────

function handleAddInstinct(args) {
  const {
    trigger,
    action,
    confidence,
  ...
  if (!trigger?.trim()) {
    return { isError: true, content: [{ type: 'text', text: '缺少必填參數: trigger' }] };
  }
  if (!action?.trim()) {
    return { isError: true, content: [{ type: 'text', text: '缺少必填參數: action' }] };
  }
  if (confidence == null || isNaN(Number(confidence))) {
    return { isError: true, content: [{ type: 'text', text: '缺少必填參數: confidence（必須為 0.0~1.0 數字）' }] };
  }
  const confNum = Number(confidence);
  if (confNum < 0 || confNum > 1) {
    return { isError: true, content: [{ type: 'text', text: 'INST_001: confidence 必須介於 0.0~1.0' }] };
  }
  if (!scope?.trim()) {
    return { isError: true, content: [{ type: 'text', text: '缺少必填參數: scope' }] };
  }
  if (!['session', 'pipeline-session', 'project', 'global'].includes(scope)) {
    return { isError: true, content: [{ type: 'text', text: `INST_002: scope 必須為 session/pipeline-session/project/global，收到: ${scope}` }] };
  }
  if (verifier_status != null && !['approved', 'rejected', 'needs-more-evidence'].includes(verifier_status)) {
    return { isError: true, content: [{ type: 'text', text: `INST_003: verifier_status 必須為 approved/rejected/needs-more-evidence` }] };
  }
  if (evidence_jsonb != null) {
    try { JSON.parse(evidence_jsonb); }
    catch { return { isError: true, content: [{ type: 'text', text: 'INST_004: evidence_jsonb 必須為合法 JSON 字串' }] }; }
  }
  ...
  try {
    ...
    // Search-first: check if (trigger, scope) already exists
    const existing = database.prepare(
      'SELECT id FROM instincts WHERE trigger = ? AND scope = ?'
    ).get(trigger.trim(), scope.trim());

    // Generate ID: inst_ + slug(trigger, 20 chars) + _ + scope slug
    ...
    // Ledger dual-write (對齊 handleAddIntentionalDecision 範式)
    appendLedger('instincts', status === 'created' ? 'add' : 'update', {
      id, trigger: trigger.trim(), scope: scope.trim(), confidence: confNum,
      domain: domain || null, created_at: now,
    });

    return {
      content: [{ type: 'text', text: JSON.stringify({ id, status, trigger: trigger.trim(), scope: scope.trim(), confidence: confNum }) }],
    };
  } catch (err) {
    process.stderr.write(`[phycool-context] add_instinct error: ${err.message}\n`);
    return { isError: true, content: [{ type: 'text', text: `add_instinct 失敗：${err.message}` }] };
  }
}

function handleSearchInstincts(args) {
  const {
    query = '',
    scope = null,
    min_confidence = null,
  ...
  try {
    ...
    process.stderr.write(`[phycool-context] search_instincts error: ${err.message}\n`);
    return { isError: true, content: [{ type: 'text', text: `search_instincts 失敗：${err.message}` }] };
  }
}

function handleDecayInstinct(args) {
  const { id, amount = 0.1 } = args || {};

  if (!id?.trim()) {
    return { isError: true, content: [{ type: 'text', text: '缺少必填參數: id' }] };
  }
  ...
  try {
    const row = database.prepare('SELECT id, confidence FROM instincts WHERE id = ?').get(id.trim());
    if (!row) {
      return { isError: true, content: [{ type: 'text', text: `INST_010: Instinct id=${id} 不存在` }] };
    }

    const newConf = Math.max(0, row.confidence - decayAmount);
    const now = getTaiwanTimestamp();

    // 若衰減後 confidence < 0.3 則設定 decay_at
    const decayAt = newConf < 0.3 ? now : null;
    if (decayAt) {
      database.prepare(
        'UPDATE instincts SET confidence = ?, last_seen = ?, decay_at = ? WHERE id = ?'
      ).run(newConf, now, decayAt, id.trim());
    } else {
      database.prepare(
        'UPDATE instincts SET confidence = ?, last_seen = ? WHERE id = ?'
      ).run(newConf, now, id.trim());
    }

    // Ledger dual-write
    appendLedger('instincts', 'decay', {
      id: id.trim(), old_confidence: row.confidence, new_confidence: newConf,
      decay_amount: decayAmount, decay_at: decayAt, last_seen: now,
    });

    return {
      content: [{ type: 'text', text: JSON.stringify({ id: id.trim(), new_confidence: newConf, decay_at: decayAt }) }],
    };
  } catch (err) {
    process.stderr.write(`[phycool-context] decay_instinct error: ${err.message}\n`);
    return { isError: true, content: [{ type: 'text', text: `decay_instinct 失敗：${err.message}` }] };
  }
}

function handlePromoteInstinct(args) {
  const { id } = args || {};
  ...
  try {
    const row = database.prepare(
      'SELECT id, trigger, scope, confidence, verifier_status FROM instincts WHERE id = ?'
    ).get(id.trim());

    if (!row) {
      return { isError: true, content: [{ type: 'text', text: `INST_020: Instinct id=${id} 不存在` }] };
    }

    // 條件檢查：confidence >= 0.8 AND verifier_status = 'approved' AND scope = 'project'
    if (row.scope === 'global') {
      return {
        content: [{ type: 'text', text: JSON.stringify({ ok: false, reason: 'Instinct 已是 global scope，無需晉升', id: id.trim() }) }],
      };
    }
    if (row.scope !== 'project') {
      return {
        content: [{ type: 'text', text: JSON.stringify({ ok: false, reason: `promote 僅適用 scope=project 的 Instinct（目前 scope=${row.scope}）`, id: id.trim() }) }],
      };
    }
    if (row.confidence < 0.8) {
      return {
        content: [{ type: 'text', text: JSON.stringify({ ok: false, reason: `confidence ${row.confidence} < 0.8，未達晉升門檻`, id: id.trim(), required_confidence: 0.8 }) }],
      };
    }
    if (row.verifier_status !== 'approved') {
      return {
        content: [{ type: 'text', text: JSON.stringify({ ok: false, reason: `verifier_status="${row.verifier_status}"，promote 需 "approved"`, id: id.trim() }) }],
      };
    }

    const now = getTaiwanTimestamp();
    database.prepare(
      'UPDATE instincts SET scope = ?, last_seen = ? WHERE id = ?'
    ).run('global', now, id.trim());

    // Ledger dual-write
    appendLedger('instincts', 'promote', {
      id: id.trim(), trigger: row.trigger, from_scope: 'project', to_scope: 'global',
      confidence: row.confidence, promoted_at: now,
    });

    return {
      content: [{ type: 'text', text: JSON.stringify({ ok: true, scope: 'global', id: id.trim(), trigger: row.trigger, confidence: row.confidence }) }],
    };
  } catch (err) {
    process.stderr.write(`[phycool-context] promote_instinct error: ${err.message}\n`);
    return { isError: true, content: [{ type: 'text', text: `promote_instinct 失敗：${err.message}` }] };
  }
}

// ──────────────────────────────────────────────
// whp-5-message-bus-mcp: 4 個 worker protocol thin handler
// 驗參數 → 委派 .context-db/scripts/worker-protocol-ops.js → 塑形回應 → 寫入 tool 附 ledger。
// CAS 零筆結果（result.isError 未設）維持成功形狀，不設 MCP isError（BR-031）。
// ──────────────────────────────────────────────
function handleSearchWorkerRuns(args) {
  try {
    const result = searchWorkerRuns(args || {});
    return { content: [{ type: 'text', text: JSON.stringify(result) }] };
  } catch (err) {
    process.stderr.write(`[phycool-context] search_worker_runs error: ${err.message}\n`);
    return { isError: true, content: [{ type: 'text', text: `WHP5-E04: search_worker_runs 失敗（DB 無法開啟或 SQL 錯誤）：${err.message}` }] };
  }
}

function handleAckWorkerRun(args) {
  try {
    const result = ackWorkerRun(args || {}, { onWrite: appendLedger });
    if (result.isError) {
      return { isError: true, content: [{ type: 'text', text: `${result.code}: ${result.reason}` }] };
    }
    return { content: [{ type: 'text', text: JSON.stringify(result) }] };
  } catch (err) {
    process.stderr.write(`[phycool-context] ack_worker_run error: ${err.message}\n`);
    return { isError: true, content: [{ type: 'text', text: `WHP5-E04: ack_worker_run 失敗（DB 無法開啟或 SQL 錯誤，未寫入任何列）：${err.message}` }] };
  }
}

function handleGateWorkerRun(args) {
  try {
    const result = gateWorkerRun(args || {}, { onWrite: appendLedger });
    if (result.isError) {
      return { isError: true, content: [{ type: 'text', text: `${result.code}: ${result.reason}` }] };
    }
    return { content: [{ type: 'text', text: JSON.stringify(result) }] };
  } catch (err) {
    process.stderr.write(`[phycool-context] gate_worker_run error: ${err.message}\n`);
    return { isError: true, content: [{ type: 'text', text: `WHP5-E04: gate_worker_run 失敗（DB 無法開啟或 SQL 錯誤，交易已回滾未寫入任何列）：${err.message}` }] };
  }
}

function handleAddWorkerMessage(args) {
  try {
    const result = addWorkerMessage(args || {}, { onWrite: appendLedger });
    if (result.isError) {
      return { isError: true, content: [{ type: 'text', text: `${result.code}: ${result.reason}` }] };
    }
    return { content: [{ type: 'text', text: JSON.stringify(result) }] };
  } catch (err) {
    process.stderr.write(`[phycool-context] add_worker_message error: ${err.message}\n`);
    return { isError: true, content: [{ type: 'text', text: `WHP5-E04: add_worker_message 失敗（SQLITE_BUSY 重試耗盡或 SQL 錯誤，未寫入任何列）：${err.message}` }] };
  }
}

// ──────────────────────────────────────────────
// ccb-1-db-mcp-import: 4 個聊天室通道 thin handler
// 驗參數 → 委派 .context-db/scripts/ctrl-channel-ops.js → 塑形回應 → 寫入 tool 附 ledger。
// CAS 零筆結果（result.isError 未設）維持成功形狀，不設 MCP isError（BR-013/BR-023/BR-026）。
// ──────────────────────────────────────────────
function handlePostCtrlMessage(args) {
  try {
    const result = postCtrlMessage(args || {}, { onWrite: appendLedger });
    if (result.isError) {
      return { isError: true, content: [{ type: 'text', text: `${result.code}: ${result.reason}` }] };
    }
    return { content: [{ type: 'text', text: JSON.stringify(result) }] };
  } catch (err) {
    process.stderr.write(`[phycool-context] post_ctrl_message error: ${err.message}\n`);
    return { isError: true, content: [{ type: 'text', text: `CCB1-E04: post_ctrl_message 失敗（DB 無法開啟或 SQL 錯誤）：${err.message}` }] };
  }
}

function handleReadCtrlMessages(args) {
  try {
    const result = readCtrlMessages(args || {}); // BR-043: read has no ledger — matches handleSearchWorkerRuns precedent
    if (result.isError) {
      return { isError: true, content: [{ type: 'text', text: `${result.code}: ${result.reason}` }] };
    }
    return { content: [{ type: 'text', text: JSON.stringify(result) }] };
  } catch (err) {
    process.stderr.write(`[phycool-context] read_ctrl_messages error: ${err.message}\n`);
    return { isError: true, content: [{ type: 'text', text: `CCB1-E04: read_ctrl_messages 失敗（DB 無法開啟或 SQL 錯誤）：${err.message}` }] };
  }
}

function handleCloseCtrlThread(args) {
  try {
    const result = closeCtrlThread(args || {}, { onWrite: appendLedger });
    if (result.isError) {
      return { isError: true, content: [{ type: 'text', text: `${result.code}: ${result.reason}` }] };
    }
    return { content: [{ type: 'text', text: JSON.stringify(result) }] };
  } catch (err) {
    process.stderr.write(`[phycool-context] close_ctrl_thread error: ${err.message}\n`);
    return { isError: true, content: [{ type: 'text', text: `CCB1-E04: close_ctrl_thread 失敗（DB 無法開啟或 SQL 錯誤）：${err.message}` }] };
  }
}

function handleUpdateCtrlBoard(args) {
  try {
    const result = updateCtrlBoard(args || {}, { onWrite: appendLedger });
    if (result.isError) {
      return { isError: true, content: [{ type: 'text', text: `${result.code}: ${result.reason}` }] };
    }
    return { content: [{ type: 'text', text: JSON.stringify(result) }] };
  } catch (err) {
    process.stderr.write(`[phycool-context] update_ctrl_board error: ${err.message}\n`);
    return { isError: true, content: [{ type: 'text', text: `CCB1-E04: update_ctrl_board 失敗（DB 無法開啟或 SQL 錯誤）：${err.message}` }] };
  }
}
...
async function main() {
  ...
  try {
    getDb();
    process.stderr.write('[phycool-context] DB connected: ' + DB_PATH + '\n');
  } catch (err) {
    process.stderr.write('[phycool-context] WARNING: ' + err.message + '\n');
    process.stderr.write('[phycool-context] Server will start but queries will fail until DB is initialized.\n');
  }

  const transport = new StdioServerTransport();
  await server.connect(transport);
  process.stderr.write('[phycool-context] MCP Server ready (stdio transport)\n');

  // LOW-2 fix: graceful shutdown — 等待 500ms 讓 pending embedding Promise 完成
  const gracefulShutdown = (signal) => {
    process.stderr.write(`[phycool-context] ${signal} received, shutting down...\n`);
    setTimeout(() => {
      if (db) {
        db.close();
        process.stderr.write('[phycool-context] DB connection closed\n');
      }
      process.exit(0);
    }, 500);
  };
...
// CLI entry guard — 允許 parseDateExpr 等純函式 import 復用(對齊 upsert-instinct.js:335
// 既有範式 · γ-6 回歸測試 follow-up)。直接 `node server.js` 執行時 isMainModule=true 啟動
// MCP stdio server 行為不變;被測試 import 時不啟動 server transport。
const isMainModule = import.meta.url === pathToFileURL(process.argv[1] || '').href;

if (isMainModule) {
  main().catch((err) => {
    process.stderr.write('[phycool-context] Fatal: ' + err.message + '\n');
    process.exit(1);
  });
}
