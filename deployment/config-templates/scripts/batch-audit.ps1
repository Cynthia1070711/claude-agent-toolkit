...
# [RETIRED 2026-07-28 tdb-2] C1-SprintStatus consistency check reads sprint-status.yaml,
# frozen read-only 2026-07-28 — results would reflect a permanently stale snapshot.
# Superseded by DB-based consistency checks (search_stories / story-lifecycle-invariants.md
# I1-I9 validators already wired into dev-story/code-review). Kept for historical reference only.
# ============================================================================
...
function Test-C3-Metadata {
    ...
    $issues = @()
    $fixFields = @{}
    # Pipeline model 配置: create=opus, dev=sonnet, review=opus
    if (-not $m.DevAgent) { $issues += "DEV Agent missing"; $fixFields["DevAgent"] = "CC-SONNET" }
    if (-not $m.DevTime)  { $issues += "DEV time missing"; $fixFields["DevTime"] = (Get-Date -Format "yyyy-MM-ddTHH:mm:sszzz") }
    if ($ys -eq "done" -and -not $m.ReviewAgent) { $issues += "Review Agent missing"; $fixFields["ReviewAgent"] = "CC-OPUS" }

    if ($issues.Count -eq 0) {
        return @{ Check = "C3-Metadata"; Pass = $true; Message = "DEV=$($m.DevAgent), Review=$($m.ReviewAgent)"; CanFix = $false }
    }
    return @{
        Check = "C3-Metadata"; Pass = $false; Message = ($issues -join "; ")
        CanFix = $true; FixAction = "Fill missing metadata"; FixTarget = $sf; FixFields = $fixFields
    }
}
...
function Test-H1ContainsEmoji {
    <#
    .SYNOPSIS
    使用 code point 比對 H1 行是否包含目標 emoji（避免 .Contains() 在 PowerShell 中的 encoding 問題）。
    #>
    param([string]$H1Line, [string]$TargetEmoji)
    if (-not $H1Line -or -not $TargetEmoji) { return $false }

    # 取得目標 emoji 的 code point
    $targetCodePoints = @()
    for ($ci = 0; $ci -lt $TargetEmoji.Length; $ci++) {
        if ([char]::IsHighSurrogate($TargetEmoji[$ci]) -and ($ci + 1) -lt $TargetEmoji.Length) {
            $targetCodePoints += [char]::ConvertToUtf32($TargetEmoji[$ci], $TargetEmoji[$ci + 1])
            $ci++
        } else {
            $targetCodePoints += [int]$TargetEmoji[$ci]
        }
    }

    # 掃描 H1 行的 code points
    for ($hi = 0; $hi -lt $H1Line.Length; $hi++) {
        if ([char]::IsHighSurrogate($H1Line[$hi]) -and ($hi + 1) -lt $H1Line.Length) {
            $cp = [char]::ConvertToUtf32($H1Line[$hi], $H1Line[$hi + 1])
            if ($targetCodePoints.Count -eq 1 -and $cp -eq $targetCodePoints[0]) { return $true }
        }
    }
    return $false
}
...
function Test-C7-H1Emoji {
    ...
    if ($h1 -and (Test-H1ContainsEmoji -H1Line $h1 -TargetEmoji $expected)) {
...
function Invoke-AutoFix {
    ...
    switch ($cr.Check) {
        ...
        "C3-Metadata" {
            $content = Get-Content $cr.FixTarget -Raw -Encoding UTF8
            $changed = $false
            foreach ($field in $cr.FixFields.Keys) {
                $value = $cr.FixFields[$field]
                switch ($field) {
                    "DevAgent" {
                        if ($content -match '(\|\s*\*\*DEV Agent\*\*\s*\|\s*)([^|]*?)(\s*\|)') {
                            $content = $content -replace '(\|\s*\*\*DEV Agent\*\*\s*\|\s*)[^|]*?(\s*\|)', "`${1}$value`${2}"
                            $changed = $true
                        }
                    }
                    "ReviewAgent" {
                        if ($content -match '(\|\s*\*\*Review Agent\*\*\s*\|\s*)([^|]*?)(\s*\|)') {
                            $content = $content -replace '(\|\s*\*\*Review Agent\*\*\s*\|\s*)[^|]*?(\s*\|)', "`${1}$value`${2}"
                            $changed = $true
                        }
                    }
                    "DevTime" {
                        if ($content -match '(\|\s*\*\*DEV\u5B8C\u6210\u6642\u9593\*\*\s*\|\s*)([^|]*?)(\s*\|)') {
                            $content = $content -replace '(\|\s*\*\*DEV\u5B8C\u6210\u6642\u9593\*\*\s*\|\s*)[^|]*?(\s*\|)', "`${1}$value`${2}"
                            $changed = $true
                        }
                    }
                }
            ...
            if ($changed) {
                $content | Out-File -FilePath $cr.FixTarget -Encoding UTF8 -NoNewline
                Write-Output "  [FIXED] C3: Filled metadata ($($cr.FixFields.Keys -join ', '))"
                return $true
            }
            return $false
        }
        ...
        "C7-H1Emoji" {
            ...
            if ($lines.Count -gt 0 -and $emoji) {
                $first = $lines[0]
                # 用 Unicode escape 清理已知 emoji (code point safe)
                # U+1F535 (Blue), U+1F7E0 (Orange), U+1F7E2 (Green)
                $knownEmojis = @($EmojiBlue, $EmojiOrange, $EmojiGreen)
                $cleaned = $first
                foreach ($e in $knownEmojis) {
