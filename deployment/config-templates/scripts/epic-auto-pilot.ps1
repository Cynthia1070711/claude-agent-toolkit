# ============================================================================
# PhyCool Epic Auto-Pilot Orchestrator
# [RETIRED 2026-07-28 tdb-2] Multi-engine era script (Gemini CLI dev-story assignment
# predates Single-Engine Mode FROZEN 2026-05-05). Generated prompts instruct dispatched
# agents to "Update sprint-status.yaml", which is now frozen read-only — do not invoke.
# Superseded by party-to-pipeline v5.x. Kept for historical reference only.
# ============================================================================
...
function Build-CreateStoryPrompt {
    ...
    $sid = $Story.Id
    $prompt = "You are CC-OPUS, PhyCool project AI commander.`n"
    $prompt += "Working directory: PhyCool-PCPT-MVP project root.`n`n"
...
function Build-DevStoryPrompt {
    param([PSCustomObject]$Story)
    $sid = $Story.Id
    $prompt = "You are GC-PRO, PhyCool project execution agent.`n"
    $prompt += "Working directory: PhyCool-PCPT-MVP project root.`n`n"
...
function Main {
    ...
    Write-Host "  PhyCool Epic Auto-Pilot Orchestrator" -ForegroundColor Cyan
