#!/usr/bin/env node
/**
 * audit-capability-reachability.cjs
 *
 ...
 *   - claude token減量策略研究分析/1.專案部屬必讀/**.md (deploy_hits, weight 0.20)
 ...
 *   - <  0.5 LATENT(flagged)
 *
 * 觸發: ADR-GOVERNANCE-001 + capability-integration-mandate.md(SUPREME)
 *
 * 用法:
 *   node scripts/audit-capability-reachability.cjs                    # advisory(預設)
 *   node scripts/audit-capability-reachability.cjs --json             # 機器可讀
 *   node scripts/audit-capability-reachability.cjs --md > report.md   # 人類可讀
 *   node scripts/audit-capability-reachability.cjs --strict           # 任一 latent → exit 1
 *   node scripts/audit-capability-reachability.cjs --tool search_god_nodes  # 單 capability
 *
 * Exit codes:
...
const PATHS = {
  ...
  deploy: path.join(REPO_ROOT, 'claude token減量策略研究分析/1.專案部屬必讀'),
