#!/usr/bin/env node
/**
 * audit-skill-overlap.cjs (toolkit deployment template)
 *
 ...
 *
 * 觸發: Capability Integration ADR + skill-creation-discipline.md(SUPREME)
 *
 * 部署時調整:
 ...
 *   node 1.專案部屬必讀/scripts/audit-skill-overlap.cjs --json              # 機器可讀
 *   node 1.專案部屬必讀/scripts/audit-skill-overlap.cjs --md > report.md    # 人類可讀
 *   node 1.專案部屬必讀/scripts/audit-skill-overlap.cjs --check-new <name> --description "<desc>" --triggers "t1,t2,t3"
 *                                                            # 新建檢核(對齊 §3 3 題)
 *   node 1.專案部屬必讀/scripts/audit-skill-overlap.cjs --strict            # 任一重疊 ≥ 50% → exit 1
 *   node 1.專案部屬必讀/scripts/audit-skill-overlap.cjs --skip-phase1       # 跳過 90d 0 觸發(若 DB 不可用)
 *
 * Exit codes:
...
// 部署時依本機 Memory DB 命名修改(預設對齊 toolkit context-memory.db)
const CONTEXT_DB_PATH = path.join(REPO_ROOT, '.context-db/context-memory.db');

// 部署時依本專案 SaaS skill 命名前綴修改(toolkit 預設 'pcpt-' 範例)
const PROJECT_SKILL_PREFIX = 'pcpt-';

const STOPWORDS = new Set([
  'a','an','the','of','to','in','for','on','at','by','from','with','and','or','not','is','are',
  '的','是','在','和','或','與','對','於','本','此','該','並','即','可','需','若','如','時','上','下',
  'use','when','skill','this','that','tool','function','feature','pcpt',
]);
  ...
  for (const e of entries) {
    if (e.isDirectory()) {
      ...
      if (parsed) {
        ...
        parsed.isProjectSkill = e.name.startsWith(PROJECT_SKILL_PREFIX);
        skills.push(parsed);
      }
    }
  }
...
function phase1RetireCheck(skills) {
  ...
  if (!fs.existsSync(CONTEXT_DB_PATH)) {
    result.note = `DB not found at ${CONTEXT_DB_PATH}; phase1 skipped`;
    return result;
  }
  let db;
  try {
    db = new Database(CONTEXT_DB_PATH, { readonly: true });
    // Check table exists
...
function audit(args) {
  ...
  const projectSkillCount = skills.filter(s => s.isProjectSkill).length;
  const generalCount = skills.length - projectSkillCount;
  const result = {
    timestamp: new Date().toISOString(),
    skill_count: { total: skills.length, project: projectSkillCount, general: generalCount },
    project_skill_prefix: PROJECT_SKILL_PREFIX,
    cap: { project_max: 50, general_max: 30, total_max: 80 },
    cap_status: {
      project: projectSkillCount > 50 ? 'OVER' : (projectSkillCount >= 47 ? 'NEAR_LIMIT' : 'OK'),
      general: generalCount > 30 ? 'OVER' : (generalCount >= 28 ? 'NEAR_LIMIT' : 'OK'),
      total: skills.length > 80 ? 'OVER' : (skills.length >= 75 ? 'NEAR_LIMIT' : 'OK'),
    },
  ...
  if (args.strict) {
    ...
    else if (result.cap_status.project === 'OVER' || result.cap_status.general === 'OVER' || result.cap_status.total === 'OVER') exitCode = 1;
  }
...
function renderMarkdown(result) {
  ...
  lines.push(`> Project skill prefix: ${result.project_skill_prefix}`);
  lines.push(`> Cap: project ≤ ${result.cap.project_max} / general ≤ ${result.cap.general_max} / total ≤ ${result.cap.total_max}`);
  lines.push('');
  lines.push(`## Skill Count`);
  lines.push('');
  lines.push(`- **Total**: ${result.skill_count.total} (cap status: ${result.cap_status.total})`);
  lines.push(`- **${result.project_skill_prefix}\\***: ${result.skill_count.project} (cap status: ${result.cap_status.project})`);
...
function renderConsole(result) {
  ...
  lines.push(`[skill-audit] count: total=${result.skill_count.total}(${result.cap_status.total}) ${result.project_skill_prefix}*=${result.skill_count.project}(${result.cap_status.project}) general=${result.skill_count.general}(${result.cap_status.general})`);
  if (result.new_check) {
