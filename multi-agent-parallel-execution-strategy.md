...
PhyCool 專案使用四引擎（Claude Code CLI + Gemini CLI + Antigravity IDE + Rovo Dev CLI）協作開發，且經常**同一引擎多開**（如 5 個 CC-OPUS 同時執行不同 Story）。Party Mode 討論中識別出三個核心風險：
...
```
主目錄:  PhyCool-PCPT-MVP(Antigravity)/
          ├── .git/                    (共享的 git 資料庫)
          └── .claude/worktrees/
...
```powershell
cd C:\Users\Alan\Desktop\Projects\PhyCool-PCPT-MVP(Antigravity)\.claude\worktrees\ba-3
claude              # 新對話
...
```powershell
# 回到主目錄
cd C:\Users\Alan\Desktop\Projects\PhyCool-PCPT-MVP(Antigravity)
...
```json
{
  ...
  "locks": [
    {
      "file": "src/PhyCool.Web/Program.cs",
      "agent": "CC-SONNET",
      "story": "QGR-BA-3",
      "type": "write",
...
> **關聯**: `/phycool-debt-registry` Skill v1.0.0 | TRS-34
