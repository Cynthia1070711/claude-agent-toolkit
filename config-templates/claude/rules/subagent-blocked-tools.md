...
> PhyCool 移植: `subagent-context-inject.js` SubagentStart Hook 注入
...
>
> **PhyCool 完整 OWASP 通用方法論 SSoT**: 本 3-Tier 為 subagent 簡化決策版。完整 OWASP Top 10 C# code patterns(BAD/GOOD)+ Zod→FluentValidation 對照 + Audit Triage 決策樹 + IDD-REG 自動查詢見 `/security-review §OWASP Code Patterns & Audit Triage`(G2 v2.1.0+)。
...
| **A2** | **MCP `mcp__phycool-context__search_*`**(context DB 讀取) | 查詢 SSoT 安全,有 Hook Health Check 保護 |
...
| **N6** | **ECPay Live API 直接呼叫** | _(PhyCool 特有)_ | 防未授權付款,必須 parent 確認 |
