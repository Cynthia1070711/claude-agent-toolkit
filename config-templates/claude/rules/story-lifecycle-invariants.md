...
paths:
  ...
  - ".claude/skills/phycool-create-story-depth-gate/**"
...
## Auto-Promotion Rules(由 upsert-story.js 與 record-phase-timestamp.js 強制執行)
...
| `started_at` | `ready-for-dev` | `in-progress` | dev-story 開工(v2.1.0 補;規則表**置於最後**,同 merge 同帶終態 trigger 時由前列優先命中) |

**start 側雙寫入口(v2.1.0)**:`scripts/record-phase-timestamp.js` 為直接 SQL 不經 upsert-story.js,其 `dev-start` / `review-start` 於同一 UPDATE 內以 `CASE WHEN status = ? THEN ? ELSE status END` 原子推進(dev-start:`ready-for-dev → in-progress`;review-start:`in-progress → review`),只進不退 —— fix-Rn 重派時 status=`review` 不會被倒退。`*-complete` 刻意不在該 helper 推進(workflow 收尾走 upsert-story.js merge,由本表處理,避免雙寫入口)。重跑情境(目標欄位已有值 = 該 phase 曾執行)該 helper 不覆寫時間戳,改自動 append `[rerun]` 標記至 `pipeline_notes`(v2.2.0;`--ts` backfill 除外)—— stories 時間戳維持「首次」語意,逐次精確起訖以 `worker_runs` 為 SSoT(`attempt` 未顯式指定時由 `upsert-worker-run.js` 自算 `MAX+1`,重派不撞 `ux_worker_runs_key`)。
...
| 版本 | 日期 | 變更 |
|:---:|:---:|------|
| **2.2.0** | **2026-07-29** | **重跑(斷電續跑 / create 補全 / review R2)語意落地(使用者裁定方案 3)**。stories 時間戳維持「首次」語意不覆寫;`record-phase-timestamp.js` v2.2.0 於目標欄位已有值時自動 append `[rerun]` 標記至 `pipeline_notes`(`--ts` backfill 除外),承接 agent 經三條既有注入路徑天然收到;逐次精確起訖以 `worker_runs` 為 SSoT。配套修復 `worker_runs.attempt` 硬編:`dispatch-general` 移除 `-Attempt 1`、`upsert-worker-run.js` 未顯式指定時自算 `MAX(attempt)+1` —— 原硬編下同 (story, phase) 重派必撞 `ux_worker_runs_key` UNIQUE 而 Register abort(斷電後二次派發的結構性阻斷)。驗證:rerun 隔離 probe 三斷言 + upsert-worker-run 23/23 + smoke 58/58。UI 面(看板 rerun/attempt 徽章)建 `tdb-5-stories-rerun-visibility`。 |
| **2.1.0** | **2026-07-29** | **I2/I8 start 側 enforcement gap 修復**。使用者實測回報:dev 開工寫 `started_at` 但 status 停留 `ready-for-dev`,DevConsole /stories 看板「開發中」區恆空(dev 完成由 `completed_at` 直接推 `review`,`in-progress` 從未出現)。根因兩層:(a) `scripts/record-phase-timestamp.js` 為直接 SQL 不經 upsert-story.js,只寫時間戳;(b) `AUTO_PROMOTE_RULES` 自 v1.0.0 起即缺 `started_at` 條目 —— I2 有不變量、無對應機械推進。修復:(1) record-phase-timestamp.js v2.1.0 `dev-start`/`review-start` 附帶原子 CASE 推進(只進不退);(2) upsert-story.js `AUTO_PROMOTE_RULES` 補 `started_at` 條目(置於最後,終態 trigger 優先);(3) 本檔 Auto-Promotion 表同步。驗證:bwu-7 live 修復(`started_at` preserved + `ready-for-dev → in-progress`)+ 冪等複跑無副作用 + upsert-story 既有套件 11/11 綠。全庫掃描僅 bwu-7 一筆 stale(已修),無 I3 違反。BMAD升級軌中控主視窗 hotfix,debt `TD-LIFECYCLE-DEVSTART-NO-STATUS-PROMOTION`(fixed)。走 `Skill(skill="cc-config-author")`。 |
