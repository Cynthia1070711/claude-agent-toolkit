 ...
 * @param {string} [dbPathOverride] - DB 路徑覆寫,預設為 DB_PATH(供測試注入 temp DB)
 */
function upsertDebts(annotations, storyId, dbPathOverride = DB_PATH) {
  ...
  let db;
  try {
    db = new Database(dbPathOverride);
