...
const CONTENT_MAX_BYTES = 32 * 1024;  // PhyCool DB cap per entry
const DB_PATH = path.join(__dirname, '../../.context-db/phycool.db');
...
function mapToolName(name) {
  ...
  if (/^mcp__phycool-context__/.test(name)) return 'memory';
...
function summarizeToolResult(toolName, args, content) {
  ...
  // memory (mcp__phycool-context__*)
  if (mapped === 'memory') {
    const action = toolName.replace('mcp__phycool-context__', '');
...
function processMessages(messages, sessionId) {
  if (!messages || messages.length === 0) {
    return { dedupCount: 0, summaryCount: 0, dbWritten: 0 };
  }
  ...
  return { dedupCount, summaryCount, dbWritten };
}
  ...
  process.stdin.on('end', () => {
    try {
      ...
      process.stderr.write(
        ...
        `db_written=${result.dbWritten} duration_ms=${durationMs}\n`
      );
    } catch (err) {
