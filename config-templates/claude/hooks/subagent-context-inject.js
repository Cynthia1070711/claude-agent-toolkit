 ...
 *
 * Injects PhyCool pipeline context into subagents:
...
const DB_PATH          = path.join(projectRoot, '.context-db', 'phycool.db');
...
async function main() {
  ...
  try {
    if (fs.existsSync(DB_PATH)) {
      ...
      if (fs.existsSync(betterSqlitePath)) {
        ...
        // 4.5 God Node Awareness Injection (ADR-GOVERNANCE-001 + Tianji v1.1.0)
        // 從 storyId 推斷 epic / domain,取 top-3 god node
        // 對齊 capability-integration-mandate.md Step 3 Pipeline 注入 + 補全計畫.md §6.6
        try {
          ...
          if (hasCentrality && storyId) {
            // Extract domain hint from story_id pattern (e.g., "eft-payment-xxx" → "Payment")
            // +CR R2 F8 FIXED: DOMAIN_HINTS SSoT in .context-db/lib/domain-hints.cjs
            const tokens = storyId.toLowerCase().split('-');
            const { DOMAIN_HINTS: domainHints } = require(path.join(projectRoot, '.context-db', 'lib', 'domain-hints.cjs'));
  ...
  // God Node Awareness (ADR-GOVERNANCE-001 + Tianji v1.1.0, since 2026-05-02)
  if (godNodes.length > 0) {
    ...
    parts.push(`(Use mcp__phycool-context__search_god_nodes / get_symbol_context for full context)`);
  }
