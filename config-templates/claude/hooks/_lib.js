 ...
 *
 * 2026-07-29 (bwu-5 CR / TD-HOOK-LIB-EVENT-NAME-ENV-MISSING): callers now pass their
 * registered event name explicitly. There is no CLAUDE_HOOK_EVENT_NAME in the official
 * hook environment, so the previous env read always fell through to the literal
 * 'unknown'. The env fallback is kept so any un-migrated caller keeps its old behaviour
...
function daysFromNow(n) {
  // TZ FIX 2026-05-29: 台灣日界(原 UTC 日界 → cooldown 截止日早 1 天;對齊 Constitutional Timestamp Mandate UTC+8)
