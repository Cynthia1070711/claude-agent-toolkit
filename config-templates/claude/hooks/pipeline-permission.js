 ...
 *
 * 在 Pipeline 模式下（PHYCOOL_PIPELINE_MODE=1）自動授權白名單工具，
 ...
 *
 * BR-03: PHYCOOL_PIPELINE_MODE=1 + 白名單工具 → allow
 * BR-04: 非 Pipeline 模式 → pass-through
 *
 * 安全設計：
 *   - 僅 PHYCOOL_PIPELINE_MODE=1 時啟用，避免正常開發模式被繞過
...
function evaluate(rawInput, env) {
  ...
  try {
    ...
    // BR-04: 非 Pipeline 模式 → pass-through
    if (env.PHYCOOL_PIPELINE_MODE !== '1') {
      return { output: '', exitCode: 0 };
    }
