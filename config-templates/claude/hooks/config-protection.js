 ...
 *
 * Override: 環境變數 PHYCOOL_CONFIG_OVERRIDE=1 可暫時允許修改
 *
...
function evaluate(rawInput, env) {
  ...
  try {
    ...
    // Override 機制：PHYCOOL_CONFIG_OVERRIDE=1 允許修改
    if (env.PHYCOOL_CONFIG_OVERRIDE === '1') {
    ...
    process.stderr.write(
      ...
      `  File: ${filePath}\n` +
      `  Reason: ${check.reason}\n` +
      `  To override: set PHYCOOL_CONFIG_OVERRIDE=1 (temporary, session-only)\n`
    );
