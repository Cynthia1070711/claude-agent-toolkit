<#
.SYNOPSIS
  [RETIRED 2026-07-28 — tdb-2-sprint-status-freeze-refs BR-011] Epic README auto-generator from sprint-status.yaml + epic-config.yaml
.DESCRIPTION
  RETIRED: sprint-status.yaml is frozen (# FREEZE-MARKER:tdb-2-sprint-status-freeze-refs) as of 2026-07-28 —
  its own data source (development_status: parsed below) will never change again, so any regeneration
  after this date would only reproduce a stale snapshot. The PostToolUse hook registration that invoked
  this script has been removed from .claude/settings.local.json (was also broken independent of this:
  $env:TOOL_INPUT is always empty because Claude Code hooks pass payload via stdin JSON, not that env var,
  so the "only run when sprint-status.yaml changes" filter below silently never activated — every Edit/Write
  regenerated every epic README with a pure timestamp diff). Epic-level progress is now served live from
  DevConsole /roadmap + /stories (DB-first, stories.epic_id aggregation) — see epic-config-sync/SKILL.md
  (also retired) for the companion decision record. File kept in place as historical reference; not invoked
  by any hook.
  TRS-17 (original): Transforms Epic README.md from manual maintenance to auto-generated derived view.
