...
Write-Host "  .context-db/server.js          MCP Server (23+ Tools)"
Write-Host "  .context-db/scripts/init-db.js Schema 初始化 (106 tables)"
Write-Host "  .context-db/package.json       依賴定義"
Write-Host "  .context-db/phycool.db         SQLite DB (WAL, SSoT)"
Write-Host "  .mcp.json                      MCP 註冊 (phycool-context)"
Write-Host "  .claude/rules/context-memory.md     查詢優先 + 寫入紀律"
Write-Host ""
Write-Host "MCP Tools (參見 phycool-context-memory Skill §3 完整 23 tools 列表):" -ForegroundColor Cyan
Write-Host "  search_*  : search_context / search_tech / search_stories /"
Write-Host "              search_intentional_decisions / search_debt /"
Write-Host "              search_documents / search_glossary / search_symbols /"
Write-Host "              search_god_nodes / search_conversations / semantic_search"
Write-Host "  add_*     : add_context / add_tech / add_cr_issue /"
Write-Host "              add_intentional_decision"
Write-Host "  get_*     : get_session_detail / get_patterns /"
Write-Host "              get_symbol_context / get_intentional_decision"
Write-Host "  trace_*   : trace_context"
Write-Host "  log_*     : log_workflow"
Write-Host "  upsert_*  : upsert_benchmark"
Write-Host "  verify_*  : verify_intentional_annotations"
Write-Host "  list_*    : list_sessions"
Write-Host "  detect_*  : (via gitnexus MCP)"
Write-Host ""
