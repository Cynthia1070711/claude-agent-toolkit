...
function Get-ExclusiveWritePatternsFromPolicy {
    param([string]$PolicyPath)
    $patterns = @()
    if (-not (Test-Path $PolicyPath)) { return $patterns }
    ...
    try {
        $lines = Get-Content $PolicyPath -Encoding UTF8
        for ($i = 0; $i -lt $lines.Count; $i++) {
            if ($lines[$i] -match '^\s+policy:\s*exclusive_write\s*$') {
                for ($j = $i - 1; $j -ge 0 -and $j -ge ($i - 3); $j--) {
                    if ($lines[$j] -match '^\s+-?\s*pattern:\s*"([^"]+)"') {
                        $patterns += $Matches[1]
                        break
...
# Priority: specific exclusive_write entries override general free patterns
$exclusivePatterns = Get-ExclusiveWritePatternsFromPolicy -PolicyPath $PolicyFile
$isExclusiveForce = $false
foreach ($pattern in $exclusivePatterns) {
    if ($RelPath -match (Convert-GlobToRegex -Pattern $pattern)) {
        $isExclusiveForce = $true
        break
    }
}

if (-not $isExclusiveForce -and (Test-FreePolicy -Path $RelPath)) {
    exit 0
}
