...
function Get-ProjectRoot {
    # Walk up from PSScriptRoot looking for .claude directory
    $dir = $PSScriptRoot
...
function Update-Tracker {
    ...
    # Find prior entry for same story+phase (preserve started_at + window_title across status transitions)
    $prior = $arr | Where-Object { $_.story_id -eq $StoryId -and $_.phase -eq $Phase } | Select-Object -First 1
    $arr   = @($arr | Where-Object { $_.story_id -ne $StoryId -or $_.phase -ne $Phase })
    ...
    # Preserve from prior if not explicitly provided (defensive -- avoid blanking on subsequent calls)
    $startedAt = if ($prior -and $prior.started_at) { $prior.started_at } else { Get-TaiwanTimestamp }
    $winTitle  = if ($WindowTitle) { $WindowTitle } elseif ($prior -and $prior.window_title) { $prior.window_title } else { "" }
    $effectiveCmd = if ($cmdLine) { $cmdLine } elseif ($prior -and $prior.cmd_line) { $prior.cmd_line } else { "" }
    # files_modified: append-merge with prior (some workers may write multiple times)
    $mergedFiles = @()
    if ($prior -and $prior.files_modified) { $mergedFiles += @($prior.files_modified) }
...
function Get-TrackerEntry {
    ...
    try {
        ...
        return ($arr | Where-Object { $_.story_id -eq $StoryId -and $_.phase -eq $Phase } | Select-Object -First 1)
    } catch { return $null }
}
...
function Stop-WorkerSafe {
    ...
    # CmdLine must reference worker-{phase}.ps1
    if ($wmiProc.CommandLine -notmatch "worker-$Phase\.ps1") {
        Write-PpLog "Stop-WorkerSafe: PID $($tracker.pid) cmdline NOT a worker-$Phase.ps1 -- ABORT kill" "ERROR"
        return $false
    }
...
# ── Build prompt context (minimal -- full Story injection happens via system prompt file) ──
...
function Get-ExpectedFileSet {
    ...
    if ($story.PSObject.Properties.Name -contains 'required_skills' -and $story.required_skills) {
        foreach ($skill in ($story.required_skills -split '[,;]')) {
            ...
            $files += ".gemini/skills/$sk/SKILL.md"
            $files += ".agent/skills/$sk/SKILL.md"
        }
    }
    # Source 5: shared write hot files (always included per CR Phase B / dev-story Step 8 pattern)
    $files += 'docs/implementation-artifacts/sprint-status.yaml'
