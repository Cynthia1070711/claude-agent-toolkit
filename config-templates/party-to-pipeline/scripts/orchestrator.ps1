...
function Invoke-MultiStoryBatch {
    ...
    foreach ($batch in $batches) {
        ...
        foreach ($sid in $batch) {
            ...
            # TD-1 fix (2026-05-11): rename $args → $dispatchArgs (avoid PowerShell automatic
            # variable collision under strict mode + Parameter advanced function).
            # Use $script: prefix for explicit script-scope param lookup.
            $dispatchArgs = @('-StoryId', $sid, '-Track', $script:Track,
                              '-MaxRetries', $script:MaxRetries,
                              '-AckTimeoutSec', $script:AckTimeoutSec,
                              '-AckWaitSec', $script:AckWaitSec,
                              '-CountdownSec', $script:CountdownSec,
                              '-WorkerTimeoutSec', $script:WorkerTimeoutSec,
                              '-WindowCloseTimeoutSec', $script:WindowCloseTimeoutSec)
            if ($script:SkipCreate) { $dispatchArgs += '-SkipCreate' }
            if ($script:SkipDev)    { $dispatchArgs += '-SkipDev' }
            if ($script:SkipReview) { $dispatchArgs += '-SkipReview' }
            & $PSCommandPath @dispatchArgs
