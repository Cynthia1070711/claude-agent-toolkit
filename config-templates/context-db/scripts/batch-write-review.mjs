// Batch write findings from a JSON array file + a report JSON file
// Usage: node batch-write-review.mjs <findings-array.json> <report.json>
...
const [findingsPath, reportPath] = process.argv.slice(2);
if (!findingsPath || !reportPath) {
  console.error('Usage: node batch-write-review.mjs <findings.json> <report.json>');
  process.exit(1);
}
...
for (const f of findings) {
  findingStmt.run({
    ...
    affected_files: f.affected_files ? JSON.stringify(f.affected_files) : null,
    regression_risk: f.regression_risk || null,
    suggested_story: f.suggested_story || null,
    engine: f.engine,
    cross_confirmed: f.cross_confirmed || 0,
    cross_engines: f.cross_engines ? JSON.stringify(f.cross_engines) : null,
    repro_steps: f.repro_steps || null,
    expected_result: f.expected_result || null,
    actual_result: f.actual_result || null,
    screenshot_before: f.screenshot_before || null,
    screenshot_after: f.screenshot_after || null,
    console_errors: f.console_errors || null,
    network_issues: f.network_issues || null,
    _nowTs: nowTs(),
  });
