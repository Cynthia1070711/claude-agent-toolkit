 ...
 *
 * Given a story_id, checks if the story's file changes affect any PhyCool skills.
...
// Grep a concept against all phycool skills
function grepSkills(concept) {
  const hits = [];
  try {
    const skillDirs = fs.readdirSync(skillsDir).filter(d =>
      d.startsWith('phycool-') && fs.existsSync(path.join(skillsDir, d, 'SKILL.md'))
    );
...
function main() {
  ...
  if (args.includes('--story-id')) {
    ...
    try {
      ...
      const db = new Database(path.join(__dirname, '../phycool.db'), { readonly: true });
