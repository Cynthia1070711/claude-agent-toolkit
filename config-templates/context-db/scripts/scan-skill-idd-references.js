// ============================================================
// PhyCool Context Memory DB — Skill IDD Reference Scanner
// DLA-09: 掃描 phycool-* Skills 中的 IDD 引用
...
const IDD_PATTERNS = [
  ...
  { name: 'phycool_idd_skill',      regex: /phycool-intentional-decisions\b/g },
];
...
function discoverSkillFiles() {
  ...
  for (const entry of entries) {
    if (!entry.isDirectory() || !entry.name.startsWith('phycool-')) continue;
...
function main() {
  ...
  if (!outputJson) {
    console.log(`\n🔍 Scanning ${skillFiles.length} phycool-* Skills for IDD references...\n`);
  }
