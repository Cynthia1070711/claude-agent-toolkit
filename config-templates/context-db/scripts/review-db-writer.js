// ============================================================
// PhyCool Review Analyst — DB 讀寫工具
...
function updateFinding(findingId, updates) {
  const db = getDb();
  // fixed_at auto-set: bug-fix-verification/SKILL.md 記載「fixed_at 由 review-db-writer.js
  // 自動設定(當 fix_status = 'fixed' 時)」,但此邏輯從未實作 —— 2026-07-27 CR 實測全 DB
  // 1108 筆 fixed 中 597 筆(54%)fixed_at 為 NULL。此處補上,呼叫端顯式給值時不覆寫。
  if (updates.fix_status === 'fixed' && !updates.fixed_at) {
    updates.fixed_at = nowTs();
  }
  const fields = Object.keys(updates).map(k => `${k} = @${k}`).join(', ');
  updates.updated_at = nowTs();
