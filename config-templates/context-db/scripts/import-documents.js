// ============================================================
// PhyCool Context Memory DB — 文檔匯入腳本
...
//
// 需求:
//   - phycool.db 已執行 init-db.js（CMI-5 schema）
...
function isExcluded(absPath) {
  ...
  if (rel.startsWith('claude token減量策略研究分析/')) {
    const keep = ['開發環境檢索架構全景/', '實施報告/', '記憶庫策略/'];
    if (!keep.some(k => rel.startsWith('claude token減量策略研究分析/' + k))) return true;
  }
  return false;
}
