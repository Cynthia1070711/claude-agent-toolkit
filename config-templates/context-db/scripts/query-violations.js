...
const DEFAULT_DB_PATH = path.resolve(__dirname, '..', 'phycool.db');
...
export function computeStats(rows, { since60dISO, since30dISO, baseline = BASELINE_30D } = {}) {
  ...
  const by_severity = [...bySeverity.entries()].map(([severity, count]) => ({ severity, count }))
    .sort((a, b) => b.count - a.count);
...
function renderTable(stats, timeline) {
  ...
  if (stats.by_severity?.length) {
    lines.push('\nBy severity:');
    for (const s of stats.by_severity) lines.push(`  ${String(s.count).padStart(3)}x  ${s.severity}`);
  }
  if (timeline?.length) {
