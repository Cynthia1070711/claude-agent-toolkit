...
### 4. Debt Registry Pull (DB-first via MCP)

> **CRITICAL:** 📋 DEBT REGISTRY PULL: Query tech_debt_items DB via search_debt MCP tool.
> registry.yaml 已廢棄（dla-04 遷移至 DB）。對齊 create-story `step-06` §8 既有寫法。

1. 呼叫 `search_debt({target_story: "{story_key}", status: "open", limit: 50})`（明示 `limit`,避免預設 20 靜默截斷多筆前置債務）
2. 記錄結果為 `{incoming_tech_debt}`

**If `{incoming_tech_debt}` 非空:**
- For EACH item: extract `fix_guidance_preview`（回傳欄位僅 200 字截斷;若需完整內容另呼叫 `search_debt({query:"{debt_id}"})` 或直接讀 DB）、`affected_files`、`severity`（回傳無 `problem_location` 欄位,不可引用此名）
- Output: ⚠️ **前置技術債 (search_debt):** [list] — These MUST be addressed during task implementation

**If `{incoming_tech_debt}` 為空:** set `{incoming_tech_debt}` = empty
