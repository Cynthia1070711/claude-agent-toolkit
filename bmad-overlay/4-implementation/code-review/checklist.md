...
## 🔍 Five-Layer Review Execution (v4.0)
...
### Security Expert (Step 3 Layer D)
- [ ] Security Expert executed with diff + project Read access
- [ ] Coverage confirmed: OWASP 12 維 + 5 PhyCool 專屬(ECPay / Key Vault / BackOffice namespace / Canvas Data / Auth-RBAC)全跑
- [ ] `{security_findings}` set(失敗時已 append "security" 至 `{failed_layers}` 且 main thread 已 fallback 補跑)

### Perf Expert (Step 3 Layer E)
- [ ] Perf Expert executed with diff + project Read access
- [ ] Coverage confirmed: .NET / EF Core / SQL Server / React 效能 10 維全跑
- [ ] `{perf_findings}` set(失敗時已 append "perf" 至 `{failed_layers}` 且 main thread 已 fallback 補跑)

### DB Schema Reviewer (Step 3 Layer F — conditional)
- [ ] `{has_db_changes}` evaluated: true → Layer F ran; false → skipped with reason(**不計為缺層**)
- [ ] If ran: Migration / PK / Index / Null / Spec / Retention 6 項全查,`{db_findings}` set

### SaaS 10-Dimension Audit (Step 3 main thread)
...
### Triage (Step 3d)
- [ ] All 7 finding sources normalized to unified Finding format
...
- [ ] **Test Quality**: Coverage >= 80%, Real assertions
...
- [ ] 每個延後項目寫入側車文件：根因分析(分類+因果鏈) → 路由決策(同Epic優先) → 寫入 tech-debt/{target}.debt.md（三層上下文：修復層+影響層+業務脈絡層）→ 當前Story新增 Tech Debt Reference（`tech_debt_items` 表為 SSoT,sprint-status.yaml 已凍結 2026-07-28 不再同步）
...
## 📝 Status Update
- [ ] Story Status updated to {{new_status}} (done/in-progress/blocked) — `stories` 表為唯一 SSoT(sprint-status.yaml 已凍結 2026-07-28)
- [ ] H1 Emoji synced: Invoke /story-status-emoji Mode A (🟢 done / 🟢🚧 done+debt / 🟡 in-progress) — N/A for DB-first Story（無 `.md`）
