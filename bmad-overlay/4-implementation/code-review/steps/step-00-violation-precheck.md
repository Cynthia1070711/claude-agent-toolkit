...
**Fail-open mode:** If the script exits non-zero (DB missing, parse error, schema mismatch), emit `⚠ Pre-check skipped (CLI error: <stderr>)` and proceed directly to `step-01-load-discover.md`. The gate is a quality nudge, not a hard block — telemetry infrastructure unavailability must not block work.
...
Then proceed directly to `step-01-load-discover.md`. No Read required.
...
Only after this acknowledgement may the workflow proceed to `step-01-load-discover.md`.
