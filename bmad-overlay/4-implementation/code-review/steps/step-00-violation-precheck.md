...
description: 'Workflow Entry Gate — Force Read top-3 hot violation rules before entering code-review workflow'
...
nextStepFile: '{workflow_path}/steps/step-00b-db-first-query.md'
---

# Step 0 — Violation Pre-check Gate (Workflow Entry Gate)

**Goal:** Internalize the top-3 most-violated rules in the last 30 days **before** beginning code-review work, by mandatory `Read` of each rule file.
...
**Phase scope:** `code-review`. Companion gate at `dev-story/steps/step-00-violation-precheck.md` covers the dev phase.

**Phase relevance:** Code-review historically holds **the highest violation density** (CR Phase A — try-fix-before-defer / skill-sync-gate / constitutional-standard rules). This pre-check is therefore especially load-bearing for CR work.

---

## STATE VARIABLES (set in this step)

- `{precheck_phase}` = `code-review` (hardcoded — phase identity is static per workflow)
...
### 1. Query top-3 code-review violations (30d rolling)

Run from repo root:

```bash
cd .context-db && node scripts/query-violations.js --phase code-review --since-days 30 --format json --limit 3
```
...
**Fail-open mode:** If the script exits non-zero (DB missing, parse error, schema mismatch), emit `⚠ Pre-check skipped (CLI error: <stderr>)` and proceed directly to `step-00b-db-first-query.md`. The gate is a quality nudge, not a hard block — telemetry infrastructure unavailability must not block work.
...
Output exactly:
```
✅ Pre-check passed: no code-review violations in 30d rolling window
```

Then proceed directly to `step-00b-db-first-query.md`. No Read required.
...
Only after this acknowledgement may the workflow proceed to `step-00b-db-first-query.md`.

---

## SUCCESS METRICS

- `query-violations.js` was invoked with `--phase code-review --since-days 30 --format json --limit 3`
...
**NEXT:** Load `step-00b-db-first-query.md`
