...
| Security Expert (3-D) | N | N | N | N |
| Perf Expert (3-E) | N | N | N | N |
| DB Reviewer (3-F) | N | N | N | N |
| SaaS Audit (main) | N | N | N | N |
| Merged (multi-source) | — | N | N | N |
| **Total** | **N** | **N** | **N** | **N** |

## Issues by SaaS Dimension
(Dynamic table: Security/Scalability/Observability/DataConsistency/MigrationIntegrity/ErrorHandling/Compliance/TestCoverage/SkillFORBIDDEN/UIBehavioral × CRITICAL/HIGH/MEDIUM/LOW)

> 十個維度須與 `step-03d-triage-merge.md` §2 三張對應表的封閉值域**逐字一致**。`UIBehavioral` 有兩個上游:main thread SaaS 第 10 維(`step-03:343`)與 Layer E `subcategory=frontend`(`step-03d` perf 對應表)。漏列任一維度 → 該維度的 finding 在本表中無列可歸。
...
`severity == "must-sync"` items → populate `{cr_must_sync_docs}` with target_story via `search_stories({epic_id, status:"backlog"})`(若空再試 `status:"ready-for-dev"`)— DB 為狀態 SSoT,sprint-status.yaml 已凍結不再查詢。
...
### 5. Sidecar Cleanup — 已退役(2026-08-04,保留編號防下游 §6+ 引用位移)

> **本步驟不再執行。** 收口實查:全庫 23 份 `tech-debt/{story_key}.debt.md` sidecar 對應 story 皆為 done 且全數保留 —— 本刪除指令自建立起從未被任何一次 CR 執行過(無機械檢核的規範靜默背離,root cause `SPEC_WITHOUT_ENFORCEMENT_DIVERGES_SILENTLY`)。裁定對齊既成實務:**sidecar 為長期資產**(step-04 Push Checklist 本就要求其存在,兩端規範自此一致)。消費 `TD-CR-STEP06-SIDECAR-CLEANUP-CONTRADICTS-23-INSTANCE-PRACTICE`。
...
| **8** | **Runtime Verification Defer Audit(2026-04-14 UI 腿 + 2026-06-13 API 腿)** | **UI 腿**:(Q1) diff 含 UI(.tsx/.css/DOM)? → 是 →(Q2)server 在跑(curl 7135/5173)? → 是 →(Q3)CR report 含 `[Chrome MCP Live Verification]` marker? 否 → **FAIL** +(Q4)跨 plan(Free+1 付費)? 否 → **FAIL**。**API 腿**:(Q1') diff 含 Controller `.cs` / `[Authorize]` / `[AdminPermission]` / policy / `IAuthorizationPolicyProvider` / `AddPolicy` / `CookieOptions` / `[Route]` / `AddAuthentication`? → 是 → 必 runtime HTTP 驗證(integration test 啟 `CustomWebApplicationFactory` 斷言 status,或 server 在跑時 curl/Chrome MCP 實打 endpoint)+ CR report 含 `[API Runtime Verification @ ts — endpoints, status]` marker? 否 → **FAIL**;**禁只 reflection 驗 attribute 字串當已測** | UI Story 必經 UI 腿;backend API/auth/policy/cookie/route Story 必經 API 腿;對應 marker 必寫入 CR report |
...
- 「reflection 驗 attribute 字串通過就算 auth/route 已測」→ auth/policy/cookie/route 改動 MUST runtime 實打 endpoint(integration test 啟 `CustomWebApplicationFactory` 斷言 status,或 curl/Chrome MCP),reflection 測宣告非行為(2026-06-13 RC-A 401 / RC-B 500 漏網 3 個月根因)
