...
description: 'Build review attack plan with five-layer dispatch + SaaS 10-dimension coverage'
...
**Goal:** Extract all ACs, tasks, and files; create comprehensive five-layer + 10-dimension SaaS review plan.
...
### 3. Five-Layer Dispatch Planning
...
| **Security Expert (3-D)** | diff + project | Security — OWASP 12 維 + 5 PhyCool 專屬深審 |
| **Perf Expert (3-E)** | diff + project | Scalability — .NET/EF Core/SQL Server/React 效能 10 維深審 |
| **DB Reviewer (3-F)** | diff(Migration/Model/Repository) | MigrationIntegrity — 僅 `{has_db_changes}` = true 時執行 |
| **SaaS Audit (3 main)** | all files | All 10 dimensions (safety net) |

Note planned layer execution order:
1. If sub-agent capable: launch 3a, 3b, 3c, 3-D, 3-E(+ 3-F if `{has_db_changes}` = true)in parallel + SaaS audit in main thread
2. If sequential fallback: 3a → 3b → 3c → 3-D → 3-E → 3-F(if applicable)→ SaaS audit
...
10. **Test Quality**: Real tests vs placeholder, coverage ≥ 80% (both Blind + Auditor)
...
- `{review_mode}` set
- Five-layer dispatch plan documented with dimension assignments (含 Layer D/E/F)
- 10-dimension SaaS review plan created
