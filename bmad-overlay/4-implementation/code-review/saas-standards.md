...
| **TestCoverage** | HIGH | Real assertions (not `Assert.True(true)`), ≥80% coverage |
...
| **coverage-minimum** | 測試覆蓋 ≥ 80% | **BLOCK** | HIGH issue |
...
> **REF:** `phycool-debt-registry` §2-2.3, `phycool-intentional-decisions` §9.4
...
> **REF:** `phycool-intentional-decisions` §9.4
...
## Zero-Finding Justification (對抗審查制度化)

> 蒸餾自 BMAD Adversarial Review 制度（W3）。防止 reviewer 輕易放行（假 clean），同時避免製造假問題。

當 code-review 完成後 **total findings == 0**（FIXED + DEFERRED + WON'T FIX + ACCEPTED + IDD 全為 0），reviewer **MUST** 輸出 Zero-Finding Justification：

- 列舉 **≥ 3 條**具體的「我驗證了 {X}，因為 {Y}（file:line evidence），結論為無問題」
- 每條須對應實際 review dimension（Security / Scalability / DataConsistency / ErrorHandling / Compliance / TestCoverage 等 8 維之一）
- 每條須含 `file:line` evidence（證明確實 Read 過該維度的 code，非憑空宣稱）

**格式範例:**
- 「驗證了 OWASP injection（Security 維），因為所有 query 走 EF parameterized（`OrderService.cs:142-156`），結論無 SQL injection 風險」
- 「驗證了 N+1（Scalability 維），因為列表查詢用 `.Include()` eager load（`DashboardService.cs:88`），結論無 N+1」
- 「驗證了交易一致性（DataConsistency 維），因為寫入走單一 `SaveChangesAsync()` transaction scope（`PaymentService.cs:205-218`），結論無 partial write」

**FORBIDDEN:**
- ❌ findings == 0 但無 Zero-Finding Justification（= 未實質審查的紅旗）
- ❌ Justification 流於形式（「都檢查過沒問題」無 dimension + 無 file:line）
- ❌ 為湊數製造假問題（Justification 目的是證明審查紮實，非強迫找碴）

**執行點:** code-review Step 5 Production Gate（Zero-Finding Gate，見 `step-05-production-gate.md` Step 5.2）
...
- ☐ `stories` 表 status / cr_* 欄位已寫入(sprint-status.yaml 已凍結 2026-07-28,不再同步)
